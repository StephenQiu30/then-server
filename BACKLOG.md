...
- [ ] `BL-DATA-01` 固定本人身份验证、导出字段/格式/范围、保留/过期、删除回执查询和注销后查询能力；[18-01](docs/plan/18-01-数据主体导出与删除回执合同执行计划.md) 已盘点现有后端数据，合同仍为 draft。依赖：账号和用途清单，以及 PRD 18 要求的法务/隐私/产品范围与保留批准；未批准前不实施导出接口。
