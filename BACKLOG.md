...
- [ ] `14-01-DELETE-01`（in_progress）已实现生成任务 `DELETE /generation-jobs/{job_id}`、访问立即撤销、迟到输出拒绝、对象版本/Provider task 清理目标快照、失败重试、源媒体引用级联、图→模型级联，以及账号注销等待 generation cleanup 完成后再删除生成记录；已有媒体/Provider/object-store 实际删除 worker、缓存副本和外部回执仍待接入。依赖：WORKER和已有19-06；验收：真实 PG 事务已验证访问先关、清理目标存活、完成后删输出元数据、源/账号级联与注销回执；CLOUD-ACC-005/009/012、PRIVACY-ACC-011。
