...
- [ ] `14-01-WORKER-01` 实现submit/query/fetch、租约/fencing、有限退避、不确定受理对账、校验后私有MinIO发布；当前已实现生成任务的获取/续租/释放、到期恢复递增 fencing token 和旧 worker 条件写入拒绝，Provider/产物发布仍未开启。依赖：DATA；验收：受理响应丢失不盲重发，worker/队列重启不丢任务，CLOUD-ACC-004/006/007/012。
