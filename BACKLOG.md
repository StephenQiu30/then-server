...
用户已选 [17-43](docs/plan/17-43-Web场景推荐与页面改版执行计划.md) 的场景优先方案 3，并明确要求接通本人真实衣橱。后端/生成 Client、Web 首页与推荐页、隔离真实 PostgreSQL 浏览器旅程和设计 QA 已完成本地开发；iOS 本地推荐与公开发布另验。

- [x] `17-43-WEB-01` 场景首页、示例影像标注、跨页导航、真实衣橱推荐和主动保存为计划；验收：[Acceptance 13 Web 切片](docs/acceptance/13-穿搭推荐验收.md#2026-09-26-web-场景推荐与真实衣橱切片)。
- [x] `17-43-TEST-01` 应用/HTTP/真实 PostgreSQL owner 与状态回归，前端静态/构建和浏览器计划/实际分离，方案 3 桌面及移动对照。
- [ ] `17-43-REL-01` 远端 CI、正式账号及公开发布门禁；本地合成数据结果不抵扣生产验收。

## Q4 完整图片与按需 GLB

来源：[14-01](docs/plan/14-01-完整穿搭图与按需三维生成执行计划.md)。需求：TRYON-REQ-001～006/008～010、AVATAR-REQ-043/044、CLOUD-REQ-007～009、OOTD-REQ-018/019；主责：后端+iOS+产品/资产。合同 `approved_with_gates`；后端 DATA/WORKER/DELETE 切片 `in_progress`，客户端、本地生成 POC 与发布门禁仍 pending。
...
- [ ] `14-01-DATA-01` 实现image/model任务、Look版本/输入快照、同意、owner、额度/成本、幂等/去重、血缘和Outbox；Provider 无关领域合同已开始并覆盖按用途输入校验、owner/purpose 作用域幂等/去重分类、纯领域接纳编排（重放/内容去重、fail-closed 预算/配额、任务+临时额度占用+单一 Outbox 意图）、额度占用只从新建 queued/未提交/未取消任务创建且时间不可倒退、额度预留身份/金额/状态版本（reserved=1、终态=2）/创建与更新时间在结算和终态重放前 fail-closed、任务核心身份/输入快照/参数哈希/状态枚举/失败原因/结果/租约/时间事实/状态修订号溢出在写入口 fail-closed、Outbox 事件身份/类型/聚合版本/用途/创建时间事实在生成时 fail-closed、不可变输出事实与 image→model 来源摘要绑定、Provider 成功回调停在 validating 且必须经过校验输出才能进入 succeeded、成功只能关联合法输出资产且失败/取消/过期不保留结果引用、取消请求后的成功终态/输出发布拒绝、成功输出与任务终态/额度消费的原子结算意图、失败状态稳定原因校验、失败/取消/过期释放与终态重放幂等且终态重放也校验提交事实、提交中/受理未知状态与显式对账防盲重、提交状态/尝试次数/时间戳/external ID 不一致时 fail-closed、活动租约内才允许提交状态/受理/Provider 状态写入且晚到受理时间不得早于任务创建、Provider 状态观察必须带有效时间且失败/取消/过期观察必须经无输出结算、过期状态写入拒绝、未知状态不得获取租约、活动租约不能直接迁移终态、外部受理/状态对账、worker 租约获取/续租/释放、过期与旧 fencing token 拒绝、活动租约内才允许发布成功输出、释放或过期后拒绝结算、终态禁止保留活动租约、成功重放要求产物发布时间不晚于任务终态、结算拒绝非法持久化租约形状、结算写集携带 token 及双账号合成 Provider 替身；当前已用隔离真实 PostgreSQL 验证参数规范化回放、幂等/内容去重、额度预留、取消、跨账号隔离、租约恢复、未知提交对账、输出成功/失败结算及终态重放；新增 Huma 实际路由测试覆盖会话、202 接纳、本人查询/分页/取消及禁用/冲突错误映射；新增真实 PostgreSQL + Huma 路由集成覆盖 202 接纳、本账号读/列表、跨账号 404、幂等回放、取消请求、删除撤权及进度查询；Provider-backed worker 全链路和 Kafka/MinIO 故障矩阵仍待 GATE/POC。依赖：11-05领域合同和POC真实参数；验收：真实PG/HTTP跨账号/版本/配额事务，CLOUD-ACC-003/008/012。
  ...
  - 2026-09-25 验收补充：依 `CLOUD-RULE-015/016`，真实 PostgreSQL 按 `(owner_id, purpose)` 分别统计活动任务、reserved/consumed quota 与 reserved 预算；成功结果计入已用额度，失败释放；同账号图片额度占满后，合法来源模型任务仍按模型独立上限接纳。隔离数据库/合成报价，无 Provider 调用或费用。见 14-01 Plan 与 Acceptance 17。
- 2026-09-25 业务闭环补充：撤回 `generation_input` 同意会在同一 PostgreSQL 事务撤销所有关联素材任务的访问并建立清理记录；确定尚未提交且没有活动租约的任务立即取消并释放 reserved quota，可能已提交任务保留取消/迟到结果清理事实。无 Provider 调用或费用，详见 14-01 Plan 与 Acceptance 17。
- 2026-09-25 业务闭环补充：用户取消仍处于 queued、`not_started` 且无外部任务 ID/worker lease 的任务时，then-server 立即原子转为 canceled 并释放内部预留 quota，额度可供下一次明确请求使用；任务已有 lease 或可能提交时继续保留取消/对账流程。无 Provider 请求或退款声明；详见 14-01 Plan 与 Acceptance 17。
- 2026-09-25 业务闭环补充：生成任务重复取消按 Plan 14-01 幂等返回当前状态；已 canceled 的任务不增加 revision、不重复释放额度，其他失败终态仍拒绝取消。无 Provider 调用；详见 14-01 Plan 与 Acceptance 17。
- 2026-09-25 验收补充：DATA-01 图片接纳现校验 `generation_input` 的 owner、人物/衣物用途分类、JPEG 摘要、ready/未删除状态、活动同意、本人/成年声明及固定规范化派生；隔离真实 PostgreSQL 覆盖跨账号、错误用途、未校验、删除、哈希冲突、撤回授权和缺少派生的拒绝路径。媒体合同进入 OpenAPI 0.29.0。全量 Go race、PostgreSQL/MinIO/Kafka/Redis integration 与 services、vet、module verify 和格式检查通过；无 Provider 调用或生成费用，DATA-01 保持 in_progress；详见 14-01 Plan 与 Acceptance 17。
- 2026-09-26 DATA-01 确认门禁补充：图片发布后由本人通过 Huma 操作明确确认，确认时间持久化且重放不变；模型接纳要求已确认图片并校验 owner、Look/revision、资产摘要和未撤权状态。隔离 PostgreSQL/MinIO 验证未确认拒绝、本人确认后模型完成、跨账号与无效状态拒绝、数据库约束；无 Provider 调用或费用。DATA-01 仍为 `in_progress`，详见 14-01 Plan 与 Acceptance 17。
- [ ] `14-01-WORKER-01` 实现submit/query/fetch、租约/fencing、有限退避、不确定受理对账、校验后私有MinIO发布；当前已实现生成任务的获取/续租/释放、到期恢复递增 fencing token、旧 worker 条件写入拒绝，`in_flight`/`unknown`/`accepted` 提交状态、显式未受理对账、external task ID、非终态 Provider 状态观察，已知未受理后的 capped exponential `next_attempt_at` 与最大提交次数门禁，验证后输出元数据与成功/失败/取消/过期结算的原子持久化，以及 Provider-neutral 清理 worker 的待办领取、目标执行、有限退避和 stale running 回收；新增 PostgreSQL `FOR UPDATE SKIP LOCKED` 的 submission/observation/result 持久领取边界，提交领取覆盖 ready 的 not_started 与 lease 过期的 in_flight（恢复后转 unknown，不重提），观察/结果只领取 accepted running / accepted validating，过期 lease 由 fencing 递增恢复；新增未启用的 submission/observation 编排分别负责带 fencing 的一次 submit 和已受理 identity 的状态观察，成功只停在 validating，终态失败/取消/过期走无输出结算；新增未启用的 result 编排只领取 validating 任务，要求 `ResultFetcher` 回传 task/look/revision/purpose/输入快照身份并校验 JPEG/GLB 元数据与 Look/revision 血缘，再原子发布/消费额度，抓取失败保留 validating；对 Fetcher 已返回当前任务固定 key+合法 version 的无效/未发布结果，现以 fencing lease 持久排队 `orphan_output` cleanup；撤权竞态并入 task/source/account manifest，账号注销也等待已关联的 orphan request 完成；MinIO 按确切版本删除，orphan 完成不删除 generation output metadata。观察 worker 对取消请求只对 queued/running/validating 外部状态调用 Cancel 并再次查询，不可信时保留取消请求；Provider queued 映射为本地 running；结果 worker 在抓取前将 validating 取消任务原子结算为 canceled 并释放额度；撤销后迟到 `external_task_id` 会在同一 PostgreSQL 事务内补进已有 cleanup manifest，并将已完成或运行中的清理请求重开为 pending，旧 cleanup claim 不能覆盖新目标，重复目标即使时间戳较旧也幂等保留；本轮补齐清理目标身份：对象目标按 `object_id + object_version_id` 去重，Provider 目标按 `provider_task_id` 去重；提交 worker 对恢复到 `in_flight` 或收到取消的 `unknown` 任务会保留待对账、释放租约且不盲目重提；清理目标在最后一次尝试失败或 stale running 恢复时均落为 `cleanup_retry_exhausted` 并从 pending/failed 队列排除，并允许在访问撤销后创建迟到的 source cleanup scope；本地 MinIO 清理 worker 已接入现有 APP_ROLE=worker|all，精确删除 manifest 记录的对象版本；provider task 保持 failed/有限重试，不发远程请求。Fetcher 写入后未返回 version 时，下一次 validating run 会先对任务固定 MinIO key 盘点并通过现有 orphan cleanup 回收已存在版本；列举/数据库失败时 fail closed；扫描后迟到写入仍需后续运行再次盘点/长期对账。真实 generation submit/query/fetch 与生产 ResultFetcher/worker 组装仍未开启；Kafka generation Outbox 唤醒已接入。依赖：DATA；验收：受理响应丢失不盲重发，明确未受理才有限退避，worker/队列重启不丢任务，清理失败可重试且旧 worker 不能覆盖新领取，CLOUD-ACC-004/006/007/012。
  - 2026-09-25 已推送 `587f323`。远端 CI `36072038147` 的 Go quality、Frontend quality、OCI image build 通过；PostgreSQL integration 与 OCI restricted-container 因 runner 的 Docker daemon 拉取 `quay.io/minio/minio` 返回 `unauthorized` 失败。该提交的本地完整 PostgreSQL/MinIO integration、race、vet 与 module verify 均通过；远端容器门禁待 registry 可用后重验。
  - 2026-09-25 验收补充：模拟 Fetcher 写入对象但未返回版本，下一次 ResultWorker 精确盘点并持久化 orphan target，不会先重复 Fetch；清理后可重试，且 key 前缀邻居未受影响。全量 race/integration、vet、module verify 与格式检查通过；无生成服务费用。提交 `66819f4` 的远端 CI `36074576586` 中 Go quality、Frontend quality 与 OCI image build 通过；PostgreSQL integration 和 OCI restricted-container 因拉取 `quay.io/minio/minio` 返回 `unauthorized` 失败。详见 Acceptance 17。
  - 2026-09-25 本地业务补充：在既有 then-server 中仅为 `GENERATION_MODE=local` 接入零费用 fixture 图片 Provider/ResultFetcher 与 PostgreSQL 轮询 Worker；API 限定 fixture image，模型、remote/local-image 服务均关闭，队列按 provider 隔离。完整 PostgreSQL+MinIO 合成任务通过 race integration、services、受限容器、vet 与构建验证。fixture 是棋盘占位图，不代表 Look 质量；POC、真实 Provider、img2threejs/GLB 与 Kafka generation 唤醒在该轮仍 pending。详见 [Plan 14-01](docs/plan/14-01-完整穿搭图与按需三维生成执行计划.md) 与 [Acceptance 17](docs/acceptance/17-云端生成与任务管理验收.md#14-01-本地-fixture-图片-worker-运行时链路)。
  - 2026-09-25 Kafka generation wake 补充：Outbox 事件进入独立 `<prefix>.generation-wake` topic，由 event worker 合并唤醒三个 PostgreSQL 持久队列；短轮询负责丢失事件/进程重启恢复。真实 Kafka topic 发布/消费验收及单测通过；无 Provider 调用/费用；14-01-WORKER-01 仍 in progress。详见 Acceptance 17。
  - 2026-09-26 取消 `validating` 任务前登记固定 MinIO key 下尚未发布的版本清理目标，避免取消直接终态留下已知孤儿；盘点失败保留可重试状态。真实 PostgreSQL + MinIO 及应用测试见 Plan 14-01 / Acceptance 17；WORKER-01 仍 `in_progress`。
  - 2026-09-26 本地 fixture 增加从已发布图片派生 model 任务、合成带纹理 GLB 的完整后端提交/观察/校验/私有发布流程；真实 PostgreSQL + MinIO 与全量 race、services/integration 通过，零 Provider 调用和供应商费用。此合成资产不是 `img2threejs` 或 Look 质量 POC；LOCAL-POC-01、WORKER-01 仍未完成。详见 Plan 14-01 / Acceptance 17。
  - 2026-09-26 WORKER-01 租约预算补充：本地 fixture 三类 worker 的租约按配置超时加 1 分钟推导；策略拒绝不足的时间余量，观察查询→取消→复查共用总截止时间。隔离 integration 和策略 race 测试覆盖，不改变外部准入或费用。WORKER-01 仍 `in_progress`，详见 Plan 14-01 / Acceptance 17。
  - 2026-09-26 本地结果保留期补充：`GENERATION_RETENTION` 从发布时间起限制读取凭据、图片确认与新模型来源；到期轮询复用任务删除事务，图的清理级联其模型，MinIO 按精确版本删除。原任务成功与额度结算不回滚，过期后新意图可重新生成。隔离 PostgreSQL+MinIO 与 HTTP 合同验证见 Plan 14-01 / Acceptance 17；真实 Provider 保留/删除仍未验收。
  - 2026-09-26 本地任务时限补充：`GENERATION_TASK_TIMEOUT` 从接纳起限制本地 fixture 的处理时间；超时 worker 只在租约结束并完成固定 key 版本盘点后，原子结算 `expired`、释放预留额度、撤权并登记精确清理目标。真实 PostgreSQL+MinIO 验证逾期、租约竞争、重复执行和对象版本删除；远端 Provider 时限与删除仍待门禁，WORKER-01 保持 `in_progress`。详见 Plan 14-01 / Acceptance 17。
...
- [ ] `14-01-DELETE-01`（in_progress）已实现生成任务 `DELETE /generation-jobs/{job_id}`、访问立即撤销、迟到输出拒绝、私有对象 key+version/Provider task 清理目标快照、失败重试、源媒体引用级联、图→模型级联、任务查询返回当前清理状态，以及账号注销等待 generation cleanup 完成后再删除生成记录；缺失历史 key 的清理 fail-closed；本地 MinIO generation cleanup worker 已接入并通过隔离 MinIO 精确版本验证；Provider/cache 删除、媒体与外部回执仍待接入。依赖：WORKER和已有19-06；验收：真实 PG 事务已验证访问先关、清理目标存活、完成后删输出元数据、源/账号级联与注销回执；CLOUD-ACC-005/009/012、PRIVACY-ACC-011。
  - 2026-09-26 本地 fixture 图片/模型任务的无远端状态 Provider 清理可完成；MinIO 继续精确版本删除，其他 Provider 仍失败关闭。隔离 PostgreSQL + MinIO 验证图→模型级联删除及账号注销回执完成。真实 Provider/cache/媒体外部回执仍待接入，DELETE-01 保持 `in_progress`。
  - 2026-09-26 Provider 清理目标补充持久化来源身份；worker 领取时核对同 owner、同任务的 Provider 和外部任务 ID，旧清单缺字段则从任务事实补齐并写回。仅凭 `fixture:` 外部 ID 不会跳过其他 Provider 的远端删除，目标冲突失败关闭；坏清单持久标记需处理且不阻塞其他用户清理。真实 Provider 删除仍待准入，DELETE-01 保持 `in_progress`。
- [x] `14-01-OUTPUT-01` then-server 已为本人已成功且未撤权的任务签发绑定固定 MinIO object version 的 5 分钟读取 URL；真实 PostgreSQL + MinIO 验证跨账号/未完成/撤权后拒绝，且签名 URL 下载返回保存时的精确版本。无 Provider/生图调用与费用。详见 14-01 Plan 与 Acceptance 17。
- [ ] `14-01-IOS-01` 冻结人物/衣物→用途/费用确认→真实任务恢复→核对保存图→显式模型→缓存复用与新版本隔离。依赖：DATA/WORKER/DELETE、17-30 TEST级共用Client、11-05实际Look/renderer及OUTPUT-01；验收：TRYON-ACC-011/012，图失败留草稿、模型失败留图。
...
- [ ] `BL-HISTORY-02` 固定云反馈/统计事实范围、关联、撤回/纠正重算与日历口径，形成数据/API/Client切片。依赖：本地16-03和当前账号实际事件；验收：无反馈不伪造统计，不混算未同步本地记录。后端 [17-34](docs/plan/17-34-账号反馈与穿着统计API执行计划.md) 已完成独立交付，本地真实 PostgreSQL 专项及包含该实现的远端 CI 结果见原 Plan；Web 页面/用户旅程接续 17-35，整体仍 `in_progress`。
