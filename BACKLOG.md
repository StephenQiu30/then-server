...
- [ ] `14-01-DATA-01` 实现image/model任务、Look版本/输入快照、同意、owner、额度/成本、幂等/去重、血缘和Outbox；Provider 无关领域合同已开始并覆盖按用途输入校验、幂等/去重分类和外部受理/状态对账，Huma/GORM 单一事实源与真实 PG/HTTP 仍待 GATE/POC。依赖：11-05领域合同和POC真实参数；验收：真实PG/HTTP跨账号/版本/配额事务，CLOUD-ACC-003/008/012。
