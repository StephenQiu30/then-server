...
- [ ] `14-01-DATA-01` 实现image/model任务、Look版本/输入快照、同意、owner、额度/成本、幂等/去重、血缘和Outbox；Provider 无关领域合同已开始并覆盖按用途输入校验、owner/purpose 作用域幂等/去重分类、纯领域接纳编排（重放/内容去重、fail-closed 预算/配额、任务+临时额度占用+单一 Outbox 意图）、不可变输出事实与 image→model 来源摘要绑定、临时额度占用的释放/结算、提交中/受理未知状态与显式对账防盲重、外部受理/状态对账及双账号合成 Provider 替身，Huma/GORM 单一事实源与真实 PG/HTTP 仍待 GATE/POC。依赖：11-05领域合同和POC真实参数；验收：真实PG/HTTP跨账号/版本/配额事务，CLOUD-ACC-003/008/012。
