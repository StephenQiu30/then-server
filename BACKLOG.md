...
来源：[17-13](docs/plan/17-13-Web账户管理执行计划.md)。主责：Web+产品；基础工程完成，账户页面合同仍draft；以下pending，不因建立backlog自动改成已批准实现。
...
- [ ] `17-13-TEST-01` 对实际业务运行生成差异、相关测试、lint/typecheck/format/build和无障碍。依赖：WEB；验收：[Acceptance17账户Web场景](docs/acceptance/17-云端生成与任务管理验收.md#账号认证与web账户管理验收)。
