...
本文件按用户要求提供一份完整的任务队列、checklist、执行顺序、建议周期和验收出口。当前按用户最新指示先在 `then-server` 实现后端与 Web 核验页，App 后续；付费调用和生产发布尚未执行。新 Look 路线与客户端云能力仍待实现。
...
- [ ] `17-32-REL-01` 回写实际证据、单独提交、工作树干净；公开入口仍依赖账号发布门禁。
