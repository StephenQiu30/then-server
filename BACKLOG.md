...
- [ ] `14-01-WORKER-01` 实现submit/query/fetch、租约/fencing、有限退避、不确定受理对账、校验后私有MinIO发布；当前已实现生成任务的获取/续租/释放、到期恢复递增 fencing token、旧 worker 条件写入拒绝，`in_flight`/`unknown`/`accepted` 提交状态、显式未受理对账、external task ID、非终态 Provider 状态观察，以及验证后输出元数据与成功/失败/取消/过期结算的原子持久化；Provider/object-store 调用与产物发布编排仍未开启。依赖：DATA；验收：受理响应丢失不盲重发，worker/队列重启不丢任务，CLOUD-ACC-004/006/007/012。
