...
- `acceptance/`：不公开的图像、录屏、网络记录与测试结果包；同目录 README 记录环境、边界和 hash。当前衣物照片网络证据位于 `acceptance/wardrobe-photo-network-20260915/`。
...
可复核的验收包不要与 DerivedData 或构建缓存混放。清理时保留被引用且仍可用的证据，移动必须校验内容 hash 并同步中央 Acceptance；已经缺失的历史文件明确标记缺失，不能用相似截图替代。研究抓取缓存、一次性修改脚本和构建产物不作为资产长期保存。
