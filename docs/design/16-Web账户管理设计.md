...
- `@umijs/openapi` 1.14.1 + TypeScript 6.0.3 已验证可消费运行时 OpenAPI 3.1.2；当前 API 0.10.0 共 31 个 operation，并包含衣橱确认属性、删除影响与账号穿搭计划请求/响应。17-13 恢复时必须从实际 `/openapi.json` 重新生成并验证函数数量，不复用旧产物。
