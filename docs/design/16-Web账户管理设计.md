...
- `@umijs/openapi` 1.14.1 + TypeScript 6.0.3 已验证可消费运行时 OpenAPI 3.1.2；当前 API 0.11.0 共 38 个 operation，并包含衣橱确认属性、覆盖计划/实际事件的删除影响、账号计划与实际穿着请求/响应。17-13 恢复时必须从实际 `/openapi.json` 重新生成并验证函数数量，不复用旧产物。
