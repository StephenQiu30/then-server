...
## 2026-09-15市场复核与生产架构定案

### 结论

Then 的三维能力采用“**离线生产资产，端内实时装配**”的单一路线：Blender 负责人物、服装、骨架、蒙皮、形变、UV 和材质的可编辑母版；Khronos glTF 导出器输出 plain glTF 2.0/GLB；App 在 Swift 层完成清单、hash、格式、rig、形变与预算校验，再交给随包 Three.js `0.185.1` / `GLTFLoader` 在隔离 WKWebView 中装配。SwiftUI 继续拥有页面、选择、草稿、保存、删除、无障碍和故障状态。

这里的“3D 建模功能”分成两个职责：

- **内容制作**发生在 App 外。模型师在 Blender 中修改可编辑源资产，服装在统一 rest pose 上绑定同一 rig，并为已批准体型参数制作各自的 shape keys。App 不提供任意网格编辑器，也不在手机上从图片重建衣服。
- **用户自定义**发生在 App 内。用户只选择内置服装、外观选项和两个有限体型参数；运行时切换独立服装网格、同步 morph、应用身体覆盖区域并旋转观察。这是可控的换装系统，不是通用建模软件。

该路线符合当前“无照片、无用户衣物也能开始”的核心场景，并保留 Woo 的人物中心、衣橱、选择托盘、Dress up、多朝向观察和 Look 记录。Woo 作者原帖只公开称其为“立体数字衣橱”，演示和公开文本没有披露 renderer、模型格式、骨架、布料求解或供应商，因此 Then 不按未公开实现做技术复制。[Woo 原帖](https://x.com/LerSentAI/status/2090783821452943404)

### GitHub 与 Firecrawl 市场调研

2026-09-15 使用 GitHub connector 读取项目 README/许可证，并用 Firecrawl 开发者索引与公开搜索交叉核对。以下结论只评价 Then 的选型，不把第三方营销声明当作 Woo 的内部事实。

| 路线/项目 | 已核对的能力与限制 | 对 Then 的决定 |
| --- | --- | --- |
| [Blender 许可](https://www.blender.org/about/license/) + [glTF 导出 API](https://docs.blender.org/api/current/bpy.ops.export_scene.html) | 官方确认 Blender 为 GPL 工具，但用户创建的 `.blend` 与其他作品归用户，可商用；当前导出 API 支持 skinning 和 morph targets | **生产事实源**。锁定本机 5.2.1 LTS，人物/服装可编辑母版与最终清理由 Blender 完成 |
| [Three.js](https://github.com/mrdoob/three.js) + `GLTFLoader` | 官方 README 将其定位为轻量、跨浏览器的 WebGL/WebGPU 三维库；MIT。glTF 可承载 mesh、PBR、skin、morph 与动画 | **保留**。当前工程已跑通本地 GLB、WebGL 2、Swift bridge 和 54 场景矩阵，无需迁移 renderer |
| [Khronos glTF-Validator](https://github.com/KhronosGroup/glTF-Validator) | 按 glTF 2.0 检查 GLB、引用、buffer、非法数值、accessor、skin/animation 与扩展 | **作为格式硬门**。另加 Then 自身的 rig、coverage、预算、来源和视觉检查 |
| [glTF Transform](https://github.com/donmccurdy/glTF-Transform) | MIT；适合对既有 glTF 做 prune、dedup、纹理缩放以及 Meshopt/KTX2 等离线优化，不是美术建模工具 | **条件工具**。MVP 不加压缩解码器；只有未压缩资产超预算时才锁版启用，且不作为 App 运行依赖 |
| [gltf-avatar-threejs](https://github.com/shrekshao/gltf-avatar-threejs) | MIT；公开演示共享 skeleton 的可换服装、身体可见区域与 GLB 合并；README 也说明其流水线需要手工处理，自定义 `gl_avatar` 扩展且服装物理仍为 TODO | **只借鉴模式**。采用共享 rig + body regions，不引入其扩展、代码或手工 JSON 流程 |
| [VRM / three-vrm](https://github.com/pixiv/three-vrm) | MIT；在 glTF 上增加 humanoid、expression、look-at、spring bone 和 MToon | **不进入 MVP**。当前没有跨应用头像交换、表情或弹簧骨需求，plain GLB 更小、更容易校验 |
| [Quaternius Universal Base Characters](https://quaternius.com/packs/universalbasecharacters.html) | 作者页面声明 CC0、共享 humanoid rig 和多发型；2026-09-15 实际下载的免费 Standard 包只有 2 个基础人物和 8 个发型，付费 Source 才包含 6 个身体、20 个发型及 rigged `.blend` | **拒绝直接作为生产人物**。可作蒙皮/发型结构研究；免费女性体约 15,060 triangles、65 joints、无动画，脸型偏成熟，缺少半马尾与衣物母版，不满足三套选定稿 |
| [Blender Human Base Meshes v1.4.1](https://www.blender.org/download/demo-files/) | Blender 官方 49 MB CC0 基础网格包；包含闭合体、四边面拓扑、face sets、UV、风格化女性身体/头/眼，最低 Blender 4.2，不含产品 rig、发型或衣服 | **选为自建母版的拓扑起点**。只取风格化女性基础网格，在项目母版内重新定型、制作头发/服装并绑定 `then-humanoid-v1`；不把基础网格原样称为完成角色 |
| [Blender Studio Ellie](https://studio.blender.org/characters/ellie/v1/) | 官方免费 CC-BY 电影角色，外观可爱；实测源文件约 129k triangles、两个 rig 分别 466/1,887 bones、66 shape keys，并带 3 个嵌入脚本，官方提示只支持 Blender 3.3～3.6 | **只作视觉和拓扑研究，不复用资产**。其青少年造型、电影级 rig/毛发和脚本依赖与当前成人角色、20-joint 和 Blender 5.2.1 合同冲突 |
| [MakeHuman](https://github.com/makehumancommunity/makehuman/blob/master/LICENSE.md) / [MPFB](https://github.com/makehumancommunity/mpfb2/blob/master/LICENSE.md) | MakeHuman 代码为 AGPL、MPFB 代码为 GPLv3；两者声明随包资产和相应输出为 CC0，第三方社区资产另算。MPFB 是 Blender 人体生成器 | **可作母版参考，不作产品依赖**。若使用只限明确的随包 CC0 输出，并登记来源；Woo 风格、拓扑、脸发与服装仍需项目自行重做 |
| [MB-Lab](https://github.com/animate1978/MB-Lab) | Blender 角色工具；其 README 明确旧分支已停止开发并迁往 Charmorph，同时列出 rig、头发等已知问题 | **不采用**。维护状态和美术产线风险高于收益 |
| [SMPL-X](https://github.com/vchoutas/smplx/blob/main/LICENSE) | 公开仓库许可证限非商业科研/教育/艺术；商业使用需另行许可 | **不采用**。首版不以受限研究模型建立商业核心资产 |
| Avatar SaaS（[Avaturn](https://avaturn.me/)、MetaPerson 等） | 厂商提供 selfie 到 3D、GLB 导出或企业 SDK；公开能力依赖网络、供应商服务与商业许可。Firecrawl 未找到 Ready Player Me 官方的当前服务状态页面，第三方迁移文案不能作为单一事实源 | **不作为主路径**。与零照片、离线可用、风格控制、成本和供应商独立目标不匹配；未来照片个性化单独评审 |
| RealityKit / USDZ | Apple 提供原生 `Model3D`、`RealityView` 和实体交互，适合 Apple 平台 3D 内容；其原生资产与工具链以 USD/Reality 为中心 | **保留为真机对照**。当前迁移会增加 GLB→USDZ 和两套资产验证，且不能直接解决服装适配，因此不并行维护生产实现 |
| Unity / Unreal / Godot / Babylon.js | 都能承担更完整场景、动画或游戏运行时；仍需同样的人物/服装母版、rig、morph 和穿模治理 | **不采用**。单人物换装不足以抵扣额外引擎、宿主、包体、构建和测试成本 |

### 2026-09-15 可编辑基础资产实测

本轮没有停在搜索摘要。三个候选均下载到仓库外的 `Then/art-source/avatar-studio/vendor/` 并使用 Blender `5.2.1 LTS` 禁止自动执行脚本后读取；运行时仓库未接入任何候选文件。

| 候选 | 本地事实 | 视觉/工程结论 |
| --- | --- | --- |
| Quaternius Standard | 官方 ZIP SHA-256 `fdbf1804c90dfc1ea03e992bff7da2dfd1a79318e13270a660180f9308455f40`；包内 `License_Standard.txt` 明确 CC0；女性 glTF 为 3 meshes、45,180 个三角索引、65-joint skin | Blender 正面渲染确认脸型偏成熟；免费发型只能近似短发和双丸子头，没有半马尾。直接采用会重复“能加载但不够可爱”的问题，判定 no-go |
| Blender Human Base Meshes 1.4.1 | 官方 ZIP SHA-256 `811f43accbb31a88266d932f8f5563b2d13586fca0ba2693aad1f5fe582b3515`；风格化女性身体为 12,502 vertices / 12,500 quads，左右眼各 802 vertices；README 明确全部资产 CC0 | 拓扑、比例和 Blender 版本符合自建母版；没有 rig、发型、服装和成品材质，判定只通过“拓扑起点”子门，不能关闭 `ASSET-01` |
| Blender Studio Ellie v1 | `.blend` SHA-256 `3dd235218725a9e68890073bbeacb4820f9b34db65d44f80c31bb88696a3c54c`；CC-BY；154 objects、119 meshes、约 129k triangles、电影级控制 rig 和嵌入脚本 | 可爱度有参考价值，但年龄观感、复杂度、版本和脚本均不符合 MVP。判定 no-go，不进入 App 或项目母版 |

Human Base Meshes 包内名为 `License` 的文本误写 Rain Rig 的 CC-BY 说明，而同包 README 与 Blender 官方下载页均明确 Human Base Meshes 为 CC0。该异常已与原包一并保留；项目来源记录同时引用官方页面、原 ZIP/hash 和 README，不以那段无关文本扩大或缩小授权。若后续授权证据发生冲突，停止分发并回到自有拓扑。

因此，三套人物不是下载三个第三方成品。单一生产母版从 Blender 官方风格化女性基础网格开始：先重塑成年但可爱的头身与脸部，再建立项目 20-joint 骨架和两项 shape keys；短发、双丸子头、半马尾由同一头部尺寸和发根合同分别制作，服装始终绑定同一身体/骨架。这个决定减少发型之外的重复资产，也避免引入旧 rig 兼容层。

仓库外已生成语义化命名的技术基础文件 `Then/art-source/avatar-studio/then-avatar-foundation-v0.1.blend`，SHA-256 为 `af3f39e32917a496bec43c1bbb772dfa84e3655013a389859b1f2484c033f916`。Blender 5.2.1 重新打开实测为 4 objects、20 bones、`Basis/shoulderWidth/torsoDepth` 三个 shape keys，身体 12,502 个顶点全部有权重且每顶点最多 4 个 influences，双眼绑定 `head` bone，不含嵌入脚本或驱动器。该文件的 `assetStatus` 为 `foundation-only`：它只固定拓扑、骨架与形变基础，没有通过成年可爱脸型、三套发型、服装、body coverage、PBR 和视觉矩阵验收，不得导出为正式 App 资产。

### 当前工程事实与判定

当前工程 POC 已从 primitive 人偶推进到 8 个本地 GLB：1 个身体、1 个脸发、2 个不同几何上装、2 个下装和 2 双鞋，共 6,000 triangles、每资产 18 joints、2 个 morph。Khronos glTF-Validator `2.0.0-dev.3.10` 对八个文件得到 0 error / 0 warning / 0 info / 0 hint；Swift 的恶意资源检查覆盖 NaN、越界 joint/index、反向绕序、外部 URI 和未知扩展；Three.js 已能在 iPhone 17 / iOS 26.5 模拟器加载并完成 54 个组合视角。

这些证据只证明格式、安全边界和运行通路，**当前资产不能作为生产模型**：

1. `asset-report.json` 明确 `reproducibleFromRepository=false`、`sourceGeneratorRetained=false`，没有可编辑 `.blend` 母版，无法从根源修正或稳定复导。
2. 人物为低多边形纯色工程夹具，脸、手、头发、衣褶、UV、PBR 纹理和材质层级达不到 Woo 参考的完整人物观感。
3. 54 场景自动化跑完不等于视觉通过；蓝色衬衫在肩/肘/袖口可见身体穿出。manifest 虽声明 `torso`、`upperArms` coverage，当前 renderer 没有实际应用身体区域遮蔽。
4. 当前 18-joint rig 可以验证站姿、装配和体型形变，但缺少左右 clavicle。生产 rig 固定为新的 `then-humanoid-v1` 共 20 joints：`root/hips/spine/chest/neck/head`，左右 `clavicle/upperArm/lowerArm/hand`，左右 `upperLeg/lowerLeg/foot`。正式资产一次性替换工程 `then-template-v1`，不维护 v1/v2 兼容层。

因此 11-02 的 54 场景记录为“运行通过、视觉失败”。2026-09-15 用户已确认统一 Woo 页面与短发、双丸子头、半马尾三套人物预设，`ASSET-01` 进入可编辑共享母版制作；穿模、材质和可复现问题从母版解决，不继续修补预编译低模网格。

### 适合当前场景的生产方案

| 层 | 固定选择 | MVP 实现 |
| --- | --- | --- |
| 人物/服装事实源 | 本机已安装 Blender `5.2.1 LTS`；随发行版的 Khronos glTF 2.0 exporter | 先交付一个风格化成人母版、一套不可关闭基础遮蔽、两上装、两下装、两双鞋；`.blend` 与原始贴图按 source revision 保存 |
| 骨架 | `then-humanoid-v1`、一个 rest pose、人物/服装相同 bind transforms | 固定 20 joints，包含左右 clavicle；不含手指、面部骨或 IK helper，直接替换 18-joint 工程 rig |
| 体型 | `shoulderWidth`、`torsoDepth` 两项受限参数 | 身体和每件服装各自制作 shape key；按 3×3 联合极值确定最终合法域，不做身高、体重或精确量体 |
| 穿模 | 资产修形 + 语义 body regions + 有限组合矩阵 | 身体按 torso/upperArms/hips/upperLegs/feet/ankles 拆成可独立隐藏区域；服装 manifest 声明 coverage，renderer 在原子切换时真正应用 |
| 材质 | 标准 PBR metallic-roughness | 第一批最多 1024px 的 baseColor/normal/ORM，先使用 GLB 内嵌 PNG/JPEG；实测超预算后再评估单一 KTX2 路径 |
| 发布格式 | plain glTF 2.0 binary `.glb`，无外部 URI、无未知扩展 | 人物和服装独立 GLB；Khronos validator + Swift 项目检查 + 视觉矩阵全部通过才进入发布清单 |
| App 运行时 | SwiftUI + Swift asset gate + Three.js `0.185.1` / WebGL 2 | 资产先在 Swift 验证并通过同源 `avatar://local/` 句柄加载；session/revision ACK、防迟到、失败保留上一有效场景 |
| 分发 | MVP GLB、贴图、缩略图随 App；编辑源不参与 App 运行 | 开发期 `.blend`/源贴图本地持久化并备份到私有 MinIO，记录对象版本与 SHA-256；App 不连接 MinIO。远程 catalog 后续另立需求 |

Blender 是离线内容工具，不进入 Go 服务、iOS 运行依赖或 Docker。仓库继续不增加资产生成脚本；导出参数、源 revision、hash、许可证与检查结果写入现有文档和 manifest，CI 直接执行已锁定的 validator/测试命令。

### 实施顺序

1. 制作并评审人物正/侧/背基准，按固定 `then-humanoid-v1` 20-joint 列表冻结 rest pose、单位、坐标和两项体型域。
2. 在同一 `.blend` 母版中完成 body regions、基础遮蔽、两项 shape keys、UV/PBR 与人工权重修正；导出 body/face-hair GLB。
3. 逐件制作两上装、两下装、两双鞋；每件复用同一 armature/rest pose，拥有自身 shape keys 和 coverage 声明。
4. 用 Khronos validator 与现有 Swift `AvatarAssetCatalog` 做格式、hash、rig、morph、绕序、预算和外联检查；错误回到 `.blend` 修复，不在 runtime 推顶点兜底。
5. 实现 renderer 的 body-region visibility，在暂存场景加载完整组合后原子切换；失败保留上一有效画面。
6. 跑两上装 × 3×3 体型 × 正/侧/背 54 场景，并人工记录穿模、轮廓、材质和角色完整度；代表组合再做连续 yaw 与参数扫动。
7. 资产关通过后继续 11-02 的持久化、WebContent/context lost、无障碍、模拟器和真机门禁。

MVP 明确不做运行时布料物理、任意衣物上传转三维、照片生成 mesh、全身扫描、VRM 互操作、表情/头发物理、远程模型商城和两套 renderer。复杂服装可以以后在 CLO/Marvelous Designer 中制作母版，但最终仍回 Blender 绑定、清理和导出；它们不是当前依赖。

## 2026-09-14核心需求定稿与建模深度分析

本节记录 2026-09-14 的需求与研究快照；涉及生产工具和当前工程状态的内容已由 2026-09-15 定案替代，产品体验范围仍然有效。
...
- 当时待定、现已于 2026-09-15 收敛：Blender 正式美术制作工具链。预编译工程 GLB 只属于 11-02 的 renderer POC，不能被当作已证明能实现 Woo 画质的产品建模方案。
...
工具决定按 Design 01 单独锁版。**上一轮“必须先把 Swift 生成器做失败才允许评估 Blender”没有工程依据，本轮取消该限制。**2026-09-15 已通过 GitHub、Firecrawl、官方 Blender API 文档与本机版本复核，将本机 Blender 5.2.1 LTS 及随发行版导出器锁为生产内容工具；不运行或新增仓库建模脚本。
...
下表保留 2026-09-14 接入 GLB 前的代码快照。2026-09-15 的实际状态、验证结果和生产判定以本文顶部“当前工程事实与判定”为准，不再把下表的“没有 GLB/loader”当作当前缺口。
...
该快照识别的根因仍然成立：Three.js API 不是当前瓶颈，正式人物和服装的可编辑事实源、视觉质量和 coverage 才是。Three.js 的 `SkinnedMesh` 需要 skeleton、每个顶点的 skin index 和 weight，常规做法是由 `GLTFLoader` 导入制作好的模型；它不会把服装 PNG 自动变成可蒙皮衣物。[Three.js SkinnedMesh](https://threejs.org/docs/pages/SkinnedMesh.html)、[GLTFLoader](https://threejs.org/docs/pages/GLTFLoader.html)
...
4. **2026-09-15 已结束 Swift 程序化人体路线。** 现有 8 个预编译 GLB 仅保留为 renderer 与恶意资源测试夹具；正式资产改用本文顶部锁定的 Blender 可编辑母版和 Khronos 导出/校验链。仓库不恢复或新增资产生成脚本，也不为旧 rig 建兼容层。
...
4. **服装母版**：正式服装在 Blender 的同一人物母版、rest pose 和骨架合同下制作并验证；Armature modifier/vertex group 只是绑定方法，自动权重之后仍需人工修正腋下、肩部、腰口、裙摆和鞋踝。当前预编译两件上装只用于工程回归，不作为正式服装母版。[Blender Armature Modifier](https://docs.blender.org/manual/en/latest/modeling/modifiers/deform/armature.html)
...
| 优先级 | 必须关闭的缺口 | 完成证据 |
| --- | --- | --- |
| P0 | 用 Blender 可编辑母版导出的正式 GLB 替换不可复现工程 GLB；冻结项目 skeleton、两件不同几何上装和有限 morph | `.blend`/源贴图/source revision、GLB/manifest/hash、正侧背、两上装 × 9 参数组合、无穿模清单 |
...
| 后端 | 已有 Go/Gin/Huma、GORM/PostgreSQL、运行时 OpenAPI 与账号 API | 本地三维不依赖后端；云生成服务仍分期实现 |
...
工程依据：[iOS 说明](https://github.com/StephenQiu30/then-app)、[RootView](https://github.com/StephenQiu30/then-app/blob/main/ThenApp/App/RootView.swift)、[后端说明](../../backend/README.md)、[后端架构](02-后端架构.md)、[现有产品计划](../plan/10-OOTD产品实施计划.md)。
...
| API / 服务端 | Huma 运行时 OpenAPI；Go + Gin + GORM + PostgreSQL | 继续使用已有模块化单体设计 | 云阶段按需实施 |
