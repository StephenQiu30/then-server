...
Redis/AMQP Go SDK 沿用下方锁版；新增 `minio/minio-go/v7 v7.3.0`（官方 module 元数据核验，Apache-2.0）用于真实协议测试。这三个 SDK 此切片仅由测试消费，不注入无实际用例的 API 依赖。Go MVS 为 SDK 更新了共享间接依赖：compress 1.19.2、cpuid/v2 2.4.0、go-toml/v2 2.3.1、x/crypto 0.55.0、x/net 0.58.0、x/text 0.41.0，go.mod/go.sum 固定实际闭包；pgx/v5 5.10.0 只由 indirect 改 direct、版本未变。既有 Go/Gin/GORM/契约库版本不变，运行回归验证这些共享依赖。镜像不等于可再分发的生产供应链验收。
