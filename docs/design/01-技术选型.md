...
| 三维显示 | Three.js + 系统 WKWebView | 已随包带 renderer；当前 avatar.js 仍以基础几何拼接，不代表标准骨架/蒙皮/morph 资产验收通过；只保留一个生产 renderer |
| API 协作 | 唯一 OpenAPI、Apple Swift OpenAPI Generator、ThenTransport、URLSession | 请求代码构建时生成；ThenTransport 只隔离生成代码，UI 和领域不拆模块；生成不依赖 Docker/API 运行 |
| Go 基线 | Go + Gin、GORM/PostgreSQL、kin-openapi、标准库 slog/context/config | 当前仅健康检查、数据库连接和内嵌 Swagger；没有业务 Service/Repository/worker，不为凑目录创建空层 |
...
| Atlas | 保留 versioned SQL 方向；当前无业务 migration | 首份服务端 schema 前核定可复现发行版及所需命令，见下文；不为工具演示创建空 SQL |
...
| 需求 | 优先复用 | 项目只实现的部分 |
| --- | --- | --- |
| 正式人物/服装制作 | Blender 与随其发行的 Khronos glTF 导出器为首选评估方向 | 风格一致母版、服装制作/权重/体型适配、发布清单；不建设通用三维编辑器或把 Swift 网格生成器做成正式美术工具 |
| 资产读入与渲染 | 锁定 Three.js 同版本 GLTFLoader、SkinnedMesh 与 morph 能力 | 资源白名单、有限骨架/槽位、会话/版本校验、生命周期和原生交互；不重写完整 glTF 解析器、蒙皮算法或任意 rig 重定向 |
| 标准资产检查 | Khronos glTF-Validator 作为离线构建工具候选 | 补项目独有的 hash、禁止外部 URI、骨架/形变一致性、组合/预算检查；标准合法不证明衣物无穿模或画质通过 |
| Swift 网络请求 | Apple 官方 OpenAPI Generator + URLSession | 领域映射、用户可恢复错误、取消；不另写请求 DTO、URL 清单或通用 HTTP 框架 |
| 数据库与事务 | GRDB、GORM、数据库约束及事务 | 实际业务 Repository、领域不变量和媒体生命周期；不实现通用 BaseRepository、UnitOfWork 框架或自动 ORM 迁移 |
| Go 运行与验证 | Gin、kin-openapi、标准库、Testcontainers | 必要的路由/授权/错误映射及真实故障测试，不引入全栈脚手架替换现有小型基线 |

Blender 与 validator 的精确分发版本、哈希及样本验证尚未闭合，仍是待实施工具建议；当前 11-02 工程夹具合同不被本节自动替换。可以直接完成工具核验和代表样本，不要求先把 Swift 方案做失败；正式制作不应继续投入通用程序化人体建模器。

### 官方资料核实与限制

- [Khronos Blender glTF 导出器](https://github.com/KhronosGroup/glTF-Blender-IO) 已随 Blender 发行，无需另找第三方通用导出插件；本轮 Blender 手册读取失败，因此只据仓库确认导出集成，不据其可能滞后的 README 判定最新 Blender 版本。
...
- [GRDB](https://github.com/groue/GRDB.swift)、[GORM 事务](https://gorm.io/docs/transactions.html) 与 [Apple OpenAPI Generator](https://github.com/apple/swift-openapi-generator) 覆盖既有数据访问/事务与构建期生成需求；当前无需因“开源优化”再并列一套 ORM 或请求库。
- [Atlas Community Edition](https://atlasgo.io/community-edition) 与标准发行版不能混称同一开源许可。当前官方页将 Community 列为 Apache-2.0、提供基本 versioned migration，而 lint/down/高级数据库对象等存在发行版差异；网页功能汇总亦有不同表述。首个 schema 切片须按实际锁版二进制验证所需命令，不能默认本机标准 CLI、Pro lint 或付费服务均属于开源免费能力。当前不升级 Atlas、不新增账号或付费依赖。
...
本轮需求以 Woo 可见体验、无上传内置换装与真实模型为核心；运行基线继续沿用本文锁定的 SwiftUI、Three.js `0.185.1` 和 WebKit，研究不自动升级依赖。GLB 工程 POC 的 Swift 确定性输出保持原合同；正式人物美术推荐可编辑母版制作，Blender/Khronos 导出校验是候选工具链，尚未新增为生产依赖。

本机只读版本查询发现 Blender `5.2.1 LTS`、build `9e2066aef7ef`；官方版本/分发完整性与锁版尚未完成，不以已安装即视为批准。原“不得引入 Blender”约束继续适用于当前产品 POC，允许直接进行工具可行性研究，取消研究文档中“先让 Swift 做失败才能评估”的额外门槛。后续选择该工具时在本文锁定分发来源、版本、exporter、参数、许可证和校验器，并同步受影响 FF-SS。

不新增 VRM、任意 rig 重定向、布料实时物理或图像生成模型作为首版依赖；这次没有依赖/模型采购、安装、代码或 API 变更。

## 2026-09-13实施顺序更新

历史决定：09-13 曾将照片生成前移；09-14 已改为无上传内置三维优先。现行范围见 [PRD 10](../prd/10-OOTD产品需求.md#2026-09-14当前批准主路径)；当前中间件启用条件以上方最小技术栈为准，照片/生成和真实三维分别验收。

## 2026-09-08 方向确认与研究边界

用户已确认可调外观与体型、可直接换装的 3D 虚拟角色方向；[`13-三维虚拟形象与服装系统研究.md`](13-三维虚拟形象与服装系统研究.md) 分析人物、服装资产和 renderer 路线，核心范围已固定，11-02 隔离 POC 契约已 approved；正式资产/生产接线尚待验收。本文仍是精确版本与启用阶段的唯一事实源，下列原批准 Three.js 边界不自动批准人体 mesh、服装适配或新的工具链；研究定案后在本文统一回写。现有 `11-01` 照片 POC 范围不变，本地 3D 换装不依赖原 2.5D 云预览的静态试穿生产前置。
...
2026-09-13 正式工程实施已通过 Debug/Release 构建、跨模块 Client 与相关照片测试、两项 UI 回归；证据及警告边界见 [17 号验收](../acceptance/17-云端生成与任务管理验收.md#2026-09-13生成client正式工程验收)。仅生成模块的技术例外已落地，其他产品门禁保持原状态。

## 2026-09-12OpenAPI文档与客户端生成链路

同日选型复核与实施授权：Docker 不是 Swagger 或 Swift 生成的必要条件。用户在 GitHub 调研后要求按 SOP 开始实现，采用 [Design 02 部署复核](02-后端架构.md#2026-09-12swagger部署方式复核) 的 Gin 内嵌方案，执行归 17-09。固定 Swagger UI v5.32.15 的官方 bundle/CSS 和许可证，通过 Go embed 打包并锁定 SHA256SUMS；不新增 npm 运行时或 Go 包装器。原独立 Docker 工具在新入口验收后直接退役，不保留兼容入口，不授权生产文档暴露。

用户要求提供 Swagger 类接口文档并生成前端请求文件。唯一 `backend/openapi.yaml` 同时供 Swagger UI 展示和 Apple Swift OpenAPI Generator 生成 `Types.swift`、`Client.swift`；不从 HTML 抓取接口，不新增注解生成的第二份契约。App 现有符号链接和 Build Tool Plugin 保持不变。
...
- 服务端：独立部署 Go API/worker，沿用 Gin、GORM、PostgreSQL、Atlas 与已有异步/媒体边界。2026-09-14 新增独立 `frontend/` Web 应用；它直接使用唯一 OpenAPI 生成 SDK，不增加 BFF 或微服务。
...
| Web OpenAPI 生成 | `@hey-api/openapi-ts` | `0.97.0`；输入仅为 `backend/openapi.yaml` |
...
- 2026-09-14 [11-02 POC](../plan/11-02-无照片三维角色与静态降级执行计划.md) 已 approved/in_progress。供应链构建固定使用本机已验证 Node.js `24.19.0` 与 npm `12.0.2`，仅从官方 npm registry 获取精确版本 `three@0.185.1` 的发布包并保留 tarball SHA-256、包内 LICENSE 与所用文件 SHA-256；运行时不包含 Node/npm，不使用 CDN。人物/服装几何继续由受控源码确定性生成；不引入 Blender 或其他建模 SaaS。
...
- `@hey-api/openapi-ts` 0.97.0 从 `backend/openapi.yaml` 生成 `src/api/generated/`。不手写 transport DTO、路径字符串或第二份 OpenAPI。0.99.0 + TypeScript 7.0.2 的失败与审计结论见 Design 16，不增加兼容补丁。
...
| 数据与迁移 | PostgreSQL + GORM Generics + Atlas；普通 CRUD 与受控 SQL 分工，数据库约束维护正确性 | 17-01 仅连接；首个云端持久数据才启用业务迁移 |
...
| GORM Generics + PostgreSQL | CRUD 降低重复代码，DB 约束维护事实；复杂查询仍须审查 SQL，不把 ORM 当迁移器 | 已有连接/版本检查；无业务 Repository 或 schema |
| Atlas versioned SQL | SQL 和 checksum 可评审、可追踪；迁移需要独立运行身份、锁/数据恢复验证 | 已选定；无业务 migration，不预建空 SQL |
| REST + OpenAPI | 单一契约连接 Swift 与 Go；成本是必须验证生成器与 JSON Schema 的实际兼容 | 已有健康契约；使用本页已批准的 ThenTransport 隔离方案，实际构建/测试与交付状态归 17-01，开发文档入口归 17-09 |
...
目录不按框架样板扩充：保持根 `main.go` + `internal`，不引入 BFF、微服务、第二个 Go module、Viper、通用 BaseRepository 或自动 DI。目录职责和标准流程见 [Design 02 目录](02-后端架构.md#目标目录)及[开发交付 SOP](02-后端架构.md#后端开发与交付-sop)。
