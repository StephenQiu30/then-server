...
- 2026-09-14 [11-02 POC](../plan/11-02-无照片三维角色与静态降级执行计划.md) 已 approved/in_progress。供应链构建固定使用本机已验证 Node.js `24.19.0` 与 npm `12.0.2`，仅从官方 npm registry 获取精确版本 `three@0.185.1` 的发布包并保留 tarball SHA-256、包内 LICENSE 与所用文件 SHA-256；运行时不包含 Node/npm，不使用 CDN。2026-09-15 正式人物/服装改由 Blender 可编辑母版生产，预编译工程 GLB 只保留回归用途；不接入建模 SaaS。
