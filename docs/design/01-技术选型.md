...
三维生产架构固定为：**Blender `5.2.1 LTS` 可编辑母版 → 随 Blender 发行的 Khronos glTF 2.0 exporter → plain GLB → Khronos glTF-Validator + Swift 项目校验 → Three.js `0.185.1` / WebGL 2 隔离运行时**。本机二进制版本已复核，Blender 仅为离线内容工具，不进入 iOS、Go、Docker 或运行时依赖；仓库不增加建模/导出脚本。
...
| 三维显示 | Three.js + 系统 WKWebView | 已随包带 renderer、GLTFLoader 和 8 个工程 GLB；格式与 54 场景运行通路已验证，正式资产视觉/coverage 未通过；只保留一个生产 renderer |
...
| 需求 | 优先复用 | 项目只实现的部分 |
| --- | --- | --- |
| 正式人物/服装制作 | 锁定 Blender 5.2.1 LTS 与随其发行的 Khronos glTF 导出器 | 风格一致母版、服装制作/权重/体型适配、发布清单；不建设通用三维编辑器或把程序化网格生成器做成正式美术工具 |
| 资产读入与渲染 | 锁定 Three.js 同版本 GLTFLoader、SkinnedMesh 与 morph 能力 | 资源白名单、有限骨架/槽位、会话/版本校验、生命周期和原生交互；不重写完整 glTF 解析器、蒙皮算法或任意 rig 重定向 |
| 标准资产检查 | Khronos glTF-Validator 作为离线构建工具 | 补项目独有的 hash、禁止外部 URI、骨架/形变一致性、组合/预算检查；标准合法不证明衣物无穿模或画质通过 |
...
Blender/validator 已按本节锁为离线生产与校验工具；代表生产样本、`.blend` 事实源、贴图、coverage 和视觉矩阵仍由 11-02 的 `ASSET-01` 验收。当前工程夹具只保留回归用途，正式制作不再投入通用程序化人体建模器。
...
本轮需求以 Woo 可见体验、无上传内置换装与真实模型为核心；运行基线继续沿用本文锁定的 SwiftUI、Three.js `0.185.1` 和 WebKit。2026-09-15 已将 Blender/Khronos 导出校验锁为离线生产工具，工程 GLB 只保留 POC/回归用途。
...
- 2026-09-15 [11-02 POC](../plan/11-02-无照片三维角色与静态降级执行计划.md) 为 approved/in_progress：工程纵切有效，统一 Woo 页面与三套人物预设已确认，正式共享母版和代表资源正在制作。供应链构建固定使用本机已验证 Node.js `24.19.0` 与 npm `12.0.2`，仅从官方 npm registry 获取精确版本 `three@0.185.1` 的发布包并保留 tarball SHA-256、包内 LICENSE 与所用文件 SHA-256；运行时不包含 Node/npm，不使用 CDN。正式人物/服装由 Blender 可编辑母版生产，预编译工程 GLB 只保留回归用途；不接入建模 SaaS。
