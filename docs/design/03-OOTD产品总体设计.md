...
- iOS 品牌资产统一位于 `Then/then-app/ThenApp/Assets.xcassets`，AppIcon 使用无透明通道的 1024 px 原图，页面标记使用 `BrandMark`；Swagger 品牌资产位于 `Then/then-server/backend/internal/adapter/httpapi/swaggerui`，只通过明确的同源只读路由提供。
