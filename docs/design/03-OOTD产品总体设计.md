...
当前多视角主路径只定义第一个可交付 MVP，不再代表完整产品取消真 3D。完整产品固定包含真实衣橱决策闭环、本人 AI 试穿、Create 360°、可交互真 3D、账号同步、导出与删除；图片、视频和 3D 分别保持真实类型。统一产品能力、页面、RealityKit/USDZ 路线、数据/API 与 P1–P6 阶段合同见 [Design 20](20-OOTD完整产品能力与阶段架构设计.md)。下文“首版”均只约束 P1；与此修正冲突的旧 mesh 否决或永久排除描述不再生效。

## 2026-09-15 Woo 多视角主路径

当前首版不再要求实时 3D mesh。用户选择短发粉色、双丸子头薄荷色或半马尾蓝色人物，再从两件上装、两件下装和两双鞋中选择；Dress up 解析已经生成、审核、发布的完整正/侧/背 `LookRenderSet`，原子更新人物舞台并允许保存。默认三套 Look 随包离线可用，其余组合从私有对象存储下载并校验。

Woo 原帖没有公开其模型、renderer 或资产格式，`立体` 不能作为 mesh 证据。当前研究与技术事实源是 [Design 18](18-Woo立体数字衣橱技术路线研究与决策.md)，页面规格是 [Design 17](17-Woo页面与三维穿搭UI设计.md)，执行是 [11-03](../plan/11-03-Woo多视角形象与内置穿搭执行计划.md)。下文 2026-09-14 及更早关于 Blender、Three.js、真实几何和体型 slider 的设计均为 11-02 历史 POC，不再控制实现。
...
- 完整 Woo 复刻目标含可见的照片/生成/下载分享/360 路径；P1 多视角切片不实现全部云页面，相关项继续标未交付。旧 PRD 的首发不导出约束保持到对应正式契约修订，不在当前切片放入可点击假入口。
...
| `WOO-REF-07` | 深色处理页、人物预览、所选衣物、Refining/Styling 文案 | 只用于真实异步 AI 任务；P1 多视角和 P5 本地真 3D 换装不显示伪造等待 |
...
flowchart TD
    ...
    VM --> Render[多视角媒体 / RealityKit Service]
