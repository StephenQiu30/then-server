...
不为未来功能创建空目录。`cmd/main.go` 已对应真实交付二进制；不会为 `api|worker|all` 分别复制入口，因为它们仍由同一配置合同和镜像承载。
