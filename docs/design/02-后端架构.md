...
`cmd/then-server/main.go` 是薄进程入口，只负责日志、调用 `internal/bootstrap` 和退出码。bootstrap 组装配置、数据库、应用用例、外部适配器与进程生命周期；业务规则按领域进入 `internal/application/<domain>`，纯领域值位于 `internal/domain`，HTTP/PostgreSQL/MinIO/RabbitMQ 实现在 `internal/adapter`。接口由 Huma operation 与 Go struct tag 声明，在运行时生成 OpenAPI 3.1.2；数据库由 PostgreSQL adapter 内 GORM record 声明，在启动监听前集中执行 `AutoMigrate`。
...
不为未来功能创建空目录。`cmd/then-server` 已对应真实交付二进制；不会为 `api|worker|all` 分别复制入口，因为它们仍由同一配置合同和镜像承载。

## 依赖方向

```mermaid
flowchart LR
    Command[cmd/then-server] --> Bootstrap[bootstrap]
...
- `cmd/then-server` 只能导入 `internal/bootstrap`；
