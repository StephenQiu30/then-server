...
## 2026-09-14客户端与服务端仓库边界

用户明确要求在本地 `Then/` 父目录下放置同级 `then-app` 与 `then-server`，并把原 iOS 与服务端拆为独立 Git 项目。本次只拆代码仓库和持续集成边界，不拆 Go 模块化单体、不引入微服务，也不改变于是 OOTD 的用户可见名称、`ThenApp` target、数据模型或 API 行为。

- `StephenQiu30/then-server` 继续保存产品级 `docs/`、唯一 `backend/openapi.yaml`、Go 服务、migration、Compose 与服务端 CI；Go module 更新为 `github.com/StephenQiu30/then-server/backend`。
- `StephenQiu30/then-app` 保存原 `app/` 的完整内容和该路径的 Git 历史，工程直接位于仓库根目录。它通过 `ThenApp/openapi.yaml` 符号链接读取同级 `then-server/backend/openapi.yaml`，不复制契约或提交生成源码。
- App CI 将两个公开仓库检出为同级目录，从而复现本地符号链接布局；服务端 CI 不检出或构建 iOS。契约变更先进入服务端，再由 App CI 编译生成 Client。
- 普通服务端 CI 固定 Go 版本，执行格式、module checksum、vet、race 单元/契约测试、Testcontainers PostgreSQL、OCI 构建与受限容器测试。需要本机 MinIO、Redis、RabbitMQ 的 `services` 测试继续作为显式集成验收，不放入每次提交。
- App CI 使用官方 `macos-26` runner 的 Xcode 26.6，执行固定版本核对、模拟器 Debug 构建和非 UI 测试；照片图库、真机、性能、隐私保护与人工视觉检查继续由对应 acceptance 管理。

该拆分不增加兼容目录、转发包或第二份契约。旧 `StephenQiu30/then` 地址由 GitHub 仓库重命名接续，代码和文档统一改用新名称；本地保留 `Then/` 作为统一父目录，其自身不作为 Git 仓库。
...
采用前后端分离的 Go 模块化单体。独立 `then-app` 负责 SwiftUI 与本地 GRDB 数据，`then-server/backend/` 通过唯一 REST/OpenAPI 契约提供后续云能力；服务端保持一个 Go module、根 main.go、一个 binary 和一个镜像，按 API/worker 角色分进程部署。仓库拆分用于独立交付和 CI，不把后端业务拆成微服务。

```mermaid
flowchart LR
    App[then-app: SwiftUI 与本地 GRDB] -. 后续明确授权的云操作 .-> HTTP[then-server: Gin / OpenAPI]
...
`Then/then-server` 顶层固定服务端 `backend/`、产品级 `docs/` 与根 Compose；iOS 顶层工程位于 `Then/then-app`。后端保持一个 Go module 和 `backend/main.go`，不增加 `server/`、`src/`、`cmd/api/` 或内部项目壳层。以下是本项目的工程选择，不是 Go 官方强制目录标准；Go 支持模块根目录命令和 `internal` 私有包，[官方布局建议](https://go.dev/doc/modules/layout)作为依据。
...
| --- | --- | --- | --- |
| SOP-00 工作区盘点 | 实现者；AGENTS、当前 Git 和工具链 | 保留已有修改，读目录说明，核对版本；记录影响 then-app/then-server/OpenAPI/DB/媒体的范围 | 工具链一致；无覆盖他人改动；已知阻断记录到切片 |
...
涉及 OpenAPI 或 App 构建边界时，在同级 `then-app` 执行 Xcode 构建：
