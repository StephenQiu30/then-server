...
- 17-20 登录用户的无图结构化衣橱 CRUD：5 个 Huma operation、GORM/PostgreSQL owner 隔离、幂等创建、revision 冲突、稳定分页和账号级联删除；App 同步仍未接入。
...
flowchart LR
    ...
    Main --> MessageQueue[messagequeue]
    Main --> Worker[worker]
...
- objectstore/messagequeue 是各自 SDK 的窄适配器，worker 只依赖 model 与最小接口；
...
### ObjectStore、MessageQueue 与 Worker

`objectstore` 只实现 MinIO 私有桶、签名、固定版本读写和全版本删除；`messagequeue` 只实现 RabbitMQ durable delivery；`worker` 编排 Outbox、JPEG 检查/重编码、Inbox 和删除。三者不把 SDK 类型扩散到 service/repository/transport，也不创建通用基础设施框架。
...
| 进程集成 | `go test -race -tags=integration ./tests` | 空 PostgreSQL 启动迁移、真实 Redis 限流、实际二进制、两项依赖断连恢复、SIGTERM |
| OCI | `go test -race -tags=container ./tests` | PostgreSQL/Redis 依赖下已构建镜像的用户、只读、资源和退出行为 |
