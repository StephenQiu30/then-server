...
`adapter/objectstore` 只实现 MinIO 私有桶、签名、固定版本读写和全版本删除；`adapter/messagequeue` 只实现 RabbitMQ durable delivery；`application/mediaworker` 通过最小 port 编排 Outbox、JPEG 检查/重编码、Inbox 和删除。SDK 类型不扩散到 domain/application/httpapi。
