...
```text
Go 1.26 + Gin + Huma + GORM + PostgreSQL 18 + Redis 8
```
...
RabbitMQ 与 MinIO 已有本机协议测试，但当前 API 不调用它们。Redis 只用于已批准的账号认证限流，不承载账号、会话或业务真相。只有具体异步任务或对象业务获批后，才新增对应 adapter 和用例，不建立空抽象。
