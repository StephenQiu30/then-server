...
当前范围补充（2026-09-16）：私人穿搭日记与图文社区按 [后端领域设计](../design/21-后端产品边界与领域设计.md) 和 [PRD 19](../prd/19-每日记录与穿搭社区需求.md) 独立定义。本文已有“首发不分享/不提供公开动态”的约束限定于原本地或生成媒体切片；私人记录仍不自动公开，新增社区必须经过独立发布确认、审核与删除合同。当前先做后端设计审查，App 与新增业务编码暂停。
...
```text
Go 1.26 + Gin + Huma + GORM + PostgreSQL 18 + Redis 8 + MinIO + RabbitMQ
```

`cmd/then-server/main.go` 是薄进程入口，只负责日志、调用 `internal/bootstrap` 和退出码。bootstrap 组装配置、数据库、应用用例、外部适配器与进程生命周期；领域模型、稳定错误、规则、用例和消费端口按业务能力归入 `internal/application/<feature>`，HTTP/PostgreSQL/MinIO/RabbitMQ 实现在 `internal/adapter`。接口由 Huma operation 与 Go struct tag 声明，在运行时生成 OpenAPI 3.1.2；数据库由 PostgreSQL adapter 内 GORM record 声明，在启动监听前集中执行 `AutoMigrate`。
...
- 17-19 合成本人照片开发闭环：8 个 Huma operation、MinIO 私有版本对象、PostgreSQL Outbox/Inbox、RabbitMQ 媒体检查/删除 worker，以及 `api|worker|all` 同二进制角色。
...
不为未来功能创建空目录。`cmd/then-server/main.go` 对应真实交付二进制；不会为 `api|worker|all` 分别复制入口，因为它们仍由同一配置合同和镜像承载。业务包间的依赖白名单与进一步拆分依据见 [Go 后端工程结构研究与规范](24-Go后端工程结构研究与规范.md)。

## 依赖方向

```mermaid
flowchart LR
    Command[cmd] --> Bootstrap[bootstrap]
    Bootstrap --> Application[application/domain modules]
...
- `cmd` 只能导入 `internal/bootstrap`；
- domain 不依赖 Gin、Huma、GORM 或外部 SDK；
- application 只能依赖 domain，不依赖 HTTP、数据库、队列、缓存或对象存储实现；
- adapter 只能依赖 domain；HTTP adapter 不得导入 GORM/消息/对象 SDK，PostgreSQL adapter 不得导入 Gin/Huma；
...
`adapter/objectstore` 只实现 MinIO 私有桶、签名、固定版本读写和全版本删除；`adapter/messagequeue` 只实现 RabbitMQ durable delivery；`application/eventworker` 通过最小 port 编排 Outbox、JPEG 检查/重编码、社区通知、Inbox 和删除。SDK 类型不扩散到 domain/application/httpapi。
...
| 本机服务 | `go test -race -tags=services ./tests/...` | `tests/services` 中的 GORM schema、真实 PG 事务/约束，以及本机 MinIO/Redis/RabbitMQ 协议 |
