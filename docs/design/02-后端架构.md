...
- `scripts/generate-ios-api.sh` 调用 Xcode 缓存的生成器并传入本地 YAML/config，没有 Docker 或 HTTP 文档输入。
...
3. iOS 通过既有符号链接与 Build Tool Plugin 生成 Types/Client。`scripts/generate-ios-api.sh` 供显式检查，独立于 Docker、文档和 API 的运行状态；不复制到源码或手改 DTO。
