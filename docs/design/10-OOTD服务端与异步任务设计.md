...
1. iOS 请求上传意图；API 完成鉴权、配额预检和用途校验。云生成输入使用独立 `generation_input` 用途、`generation-input-v1` 同意版本和 24 小时本地开发留存声明；人物输入为 `person_photo` 且需要当前本人/成年声明，衣物输入为 `ordinary_image`。二者均由账号持有，不与日记、社区或头像媒体用途互用。
...
2. 用户主动生成穿搭图时，iOS 只提交本次 Look 草稿的人物参考、选衣快照和必要资产引用；API 在接纳事务内锁定并核验每个 `generation_input` 媒体的 owner、角色对应 category、活动同意、ready 状态、固定规范化派生版本和 SHA-256 快照。iOS 使用幂等键创建图片任务，冻结输入、处理策略版本和成本预算，并在同一事务写 Outbox。
