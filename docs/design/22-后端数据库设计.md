...
| 同步变更记录（17-39 合同，待 17-40 实现） | 每 owner 的事务序列水位、`(owner_id,seq)` 唯一变更记录，实体类型/UUID/动作/可空 revision；衣橱补删除墓碑 | 同一业务事务写变更，owner 水位行锁使序列按提交顺序可读；账号删除级联清理；当前计划/实际/日记墓碑不等于完整同步，细节见 17-39 |
