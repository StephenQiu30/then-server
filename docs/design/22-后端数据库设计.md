...
| generation_jobs / generation_quota_reservations / generation_outputs / generation_cleanup_requests / generation_submission_reconciliations（14-01 后端） | 已在 then-server 现有 PostgreSQL 中保存任务、额度、固定输出、清理目标与 admin 核对审计；审计只存 actor/task/revision/attempt/decision/evidence type 与非敏感引用 | 任务和额度结算原子写入；未知受理不盲目重提；有效 worker lease 与旧 revision 拒绝核对；任务、额度与审计同事务；账号删除清理完成后删除关联审计记录 |
