...
| user_profiles 新增（19-02 部分实现） | 已实现 user_id uuid PK/FK、handle text、bio text?、revision；17-37 增加可空 avatar_media_id | lower(handle) 唯一、3–30 ASCII 字符；简介<=300 字；头像绑定须同事务校验 media owner、`profile_avatar` 用途和 ready 状态，替换/撤下先关闭旧公开读取并复用删除任务；公开 DTO 不含账号状态/邮箱/对象 key |
