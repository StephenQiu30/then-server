...
19-02 把实际表数增加到 18；19-03 增加三张日记表；19-04 增加六张帖子治理表；19-05 增加七张互动治理表；19-06 增加 `account_deletion_requests`，当前集中迁移共 35 张表。真实 PostgreSQL 已验证关系唯一、双向屏蔽、评论审核状态、通知 Outbox/Inbox 去重、申诉唯一与删除级联；可靠删号真实依赖门禁仍在执行，头像媒体列仍待后续切片。
