...
状态：`approved / implemented pending delivery`，2026-09-16；19-02～19-04 已实现账号资料、私人日记和帖子治理，19-05 已实现 B4 关系、评论、通知与申诉数据层并通过真实 PostgreSQL 验证，提交与远端 CI 待完成。[领域边界](21-后端产品边界与领域设计.md) / [接口](23-后端接口设计.md)。下表是逻辑及约束设计，不是第二份可执行 schema；GORM record 和集中 AutoMigrate 是实际建库唯一来源。
...
19-02 把实际表数增加到 18；19-03 增加三张日记表；19-04 增加六张帖子治理表；19-05 增加 `post_likes`、`post_bookmarks`、`user_follows`、`user_blocks`、`comments`、`moderation_appeals`、`notifications`，并扩展举报和治理动作的评论目标，当前集中迁移共 34 张表。真实 PostgreSQL 已验证关系唯一、双向屏蔽、评论审核状态、通知 Outbox/Inbox 去重、申诉唯一与删除级联；头像媒体列仍待后续切片。
