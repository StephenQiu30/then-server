...
状态：`approved / staged implementation`，2026-09-16；19-02 已实现账号状态与公开资料，19-03 已实现私人日记，19-04 当前工作树已实现 B3 的六张社区表与 `community_publish` 用途，B4 关系/评论/通知表仍待后续切片。[领域边界](21-后端产品边界与领域设计.md) / [接口](23-后端接口设计.md)。下表是逻辑及约束设计，不是第二份可执行 schema；GORM record 和集中 AutoMigrate 是实际建库唯一来源。
...
19-02 把实际表数增加到 18：`users` 已有 status/role/revision CHECK 与默认值；新增 `user_profiles(user_id PK/FK, handle UNIQUE, bio?, revision, timestamps)`。19-03 增加三张日记表。19-04 增加 `posts`、`post_revisions`、`post_revision_media`、`post_revision_tags`、`content_reports`、`moderation_actions`，当前集中迁移共 27 张表。真实 PostgreSQL 已验证社区至少 8 个 FK、用途/状态 CHECK、不可变版本、来源日记解绑、固定媒体版本、举报唯一、迟到审核、封禁会话和删除级联；头像媒体列及 B4 表尚未加入。
...
| content_reports（19-04 帖子目标已实现） | id uuid PK、reporter_id/post_id FK、reason/detail、status(open/resolved/dismissed)、resolution_code?、created/resolved 时间 | 同一 reporter/post 最多一个 open 举报；举报队列有界；评论/账号目标随对应切片扩展，不建兼容字段 |
