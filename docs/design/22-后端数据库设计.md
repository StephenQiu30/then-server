...
约定：`uuid` 标识；日期 `date`；绝对时间 `timestamptz`；原始时区为 IANA 文本；正文 `text`；次数 `bigint`。状态采用 `text + CHECK`，避免不断增加数据库枚举类型。JSONB 仅用于已验证的不可变配方/公开快照/有限消息，不装全部业务实体。新私有实体通常采用 `(owner_id,id)` 主键；`data_exports` 为删号后清理产物保留随机全局 ID 主键与 owner 索引，不设用户 FK，所有读取仍同时约束 owner 和 active 账号。公共实体用全局 UUID 主键并建立 `(owner_id,id)` 唯一键供所有权复合外键使用。可编辑实体通常带 created_at/updated_at 和 revision，导出任务用创建、完成、过期、撤销与清理阶段时间。客户端 owner_id 从会话得出，不能接受客户端指定另一个 owner。
...
| wardrobe_items | (owner_id,id) PK、name/category/availability/source、四项 nullable 属性、nullable archived_at、revision | 私人衣物；`archived_at` 独立表达生命周期，历史快照不回写；媒体关联仍未实现 |
...
19-02 把实际表数增加到 18；19-03 增加三张日记表；19-04 增加六张帖子治理表；19-05 增加七张互动治理表；19-06 增加 `account_deletion_requests`；17-34 增加反馈和命令回执两表；17-36 增加 `account_mail_challenges`；18-02 增加 `data_exports`，当前集中迁移共 39 张表。`data_exports` 保存 owner、档位、状态、计数/遗漏、私有对象固定版本、有限租约及过期/撤销/清理时间；不设用户 FK，使物理删号后仍可完成对象清理。18-03 扩展 `with_media` 与 `partial`，在同一只读快照按 owner 选出 ready 原图固定版本；17-37 继续纳入头像原图与资料关联。单张媒体删除事务撤销该 owner 仍可读的媒体档导出，删号事务继续撤销全部导出。清理成功后 7 天删除最小任务元数据。真实 PostgreSQL 已验证既有关系唯一、双向屏蔽、评论审核状态、通知 Outbox/Inbox 去重、申诉唯一、可靠删号、挑战一次消费、头像替换与删除级联。
...
| users 扩展（17-36 已实现） | email_verified_at timestamptz? | 修改邮箱同时清除验证状态并撤销未完成挑战；既有账号不自动标记已验证 |
| account_mail_challenges（17-36 已实现） | id uuid PK、user_id uuid FK、email、purpose text(verify_email/reset_password)、expires_at、next_attempt_at、lease_until?、attempts、sent_at?/consumed_at?/revoked_at?、时间戳 | 用户行锁串行替换同用途挑战；确认在事务中单次消费，过期不接受；令牌由服务端 HMAC 密钥按挑战重建，不存原文或 token_hash；账号删除级联清理 |
| user_profiles 新增（19-02/17-37 已实现） | user_id uuid PK/FK、handle text、bio text?、revision、可空 avatar_media_id FK | lower(handle) 唯一、3–30 ASCII 字符；简介<=300 字；头像绑定同事务校验 media owner、`profile_avatar` 用途和 ready 状态，替换/撤下先关闭旧公开读取并复用删除任务；公开 DTO 不含账号状态/邮箱/对象 key/媒体 ID |
| wardrobe_items 扩展 | `archived_at timestamptz?`（17-41 已实现）、image_media_id uuid? | 归档不覆盖 laundry/packed；候选/新计划同时检查 archived_at；复合 owner/media FK；图片只能 ready 且 wardrobe 用途 |
...
| account_deletion_requests（19-06/18-04） | id uuid PK、user_id uuid UNIQUE、status(pending/complete)、media_count、requested_at/completed_at、updated_at；18-04 增加可空 receipt_hash、receipt_expires_at、receipt_revoked_at 及 retry_observed；刻意不设 user FK | 先失效后清理；注销 202 只交付一次原始令牌，库内仅存 SHA-256；完成时固化真实重试事实；过期/撤销禁止查询，pending 内部记录留到清理完成，完成且超过 7 天的回执清除 |
| generation_jobs / generation_quota_reservations / generation_outputs / generation_cleanup_requests / generation_submission_reconciliations（14-01 后端） | 已在 then-server 现有 PostgreSQL 中保存任务、额度、固定输出、私有输出对象 key/version、清理目标与 admin 核对审计；审计只存 actor/task/revision/attempt/decision/evidence type 与非敏感引用 | 任务和额度结算原子写入；未知受理不盲目重提；有效 worker lease 与旧 revision 拒绝核对；任务、额度与审计同事务；清理 manifest 在移除输出记录前快照私有对象 key/version，key 不进入公开 DTO；账号删除清理完成后删除关联审计记录 |

18-04 给 `deletion_requests` 增加可空 `account_deletion_id`。账号删号事务将本次纳入的媒体删除请求关联至回执；进度和重试只统计这些请求，不把注销前的单张图片删除历史算入本次任务。
...
`sync_positions(owner_id,seq,seeded)` 按 owner 在业务事务内持锁分配提交水位；`sync_changes(owner_id,seq,kind,entity_id,action,revision,created_at)` 记录最小元数据，`wardrobe_item_deletions` 阻止迟到的同 ID 重建。首次读/写一次性登记现有结构化事实和已有计划/实际/日记墓碑；真实变更与连带修订同事务追加，删号级联清理。本地合成 PostgreSQL 验证见 [17-40](../plan/17-40-服务端增量同步执行计划.md)，App 主动启用及真实两设备恢复尚未验收。
