...
> 2026-09-15 范围修正：本文的多视角决定只适用于第一个零上传 MVP 切片，不再代表完整产品取消真 3D。完整产品必须交付 Woo 式多视角、本人 AI 试穿、Create 360° 与可交互真 3D；统一能力、原生 RealityKit/USDZ 路线和阶段合同见 [Design 20](20-OOTD完整产品能力与阶段架构设计.md)。
...
- 停止项：停止 11-02 的 Blender → GLB → Three.js/WKWebView 生产路线。既有工程 POC 只保留 no-go 证据，不再决定产品架构；未来资产工具由 P5 的 USDZ 合同和 POC 选择。
- 后续完整产品能力：Create 360° 使用独立异步短视频；用户本人照片试穿另行启用；可交互真 3D 按 Design 20 的 RealityKit/USDZ P5 交付，不再以“是否需要真 3D”作为待定项。
...
3. 原生 RealityKit/USDZ 在最低真机完成画质、内存、帧时间、发热、无障碍与崩溃验证。
4. 换装、连续观察、缩放、展示动作、原子缓存、坏资源恢复和多视角回退通过独立切片验收。
5. 不得把 Hunyuan3D 的静态网格、VTON360 的研究结果或 360° 视频直接当作可换装角色。

在这些条件关闭前，任何建模工具或供应商都不是生产依赖。iOS 运行时只采用 Design 20 固定的 RealityKit/USDZ，不建设 GLB、Three.js、Unity 或多 renderer 兼容分支。
