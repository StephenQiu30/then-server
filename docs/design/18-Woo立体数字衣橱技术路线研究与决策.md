...
> 2026-09-15 范围修正：本文的多视角决定只适用于第一个零上传 MVP 切片，不再代表完整产品取消真 3D。完整产品必须交付 Woo 式多视角、本人 AI 试穿、Create 360° 与可交互真 3D；统一能力、Three.js/GLB 路线和阶段合同见 [Design 20](20-OOTD完整产品能力与阶段架构设计.md)。
...
- 停止项：项目不使用 Blender 制作链。既有工程 GLB 仅作回归，Three.js 运行时由 [AVATAR-BASELINE-01](01-技术选型.md#人物技术冻结与变更规则) 固定；授权成品资产来源仍待代表包验证。
- 后续完整产品能力：Create 360° 使用独立异步短视频；用户本人照片试穿另行启用；可交互真 3D 按 Design 20 的 Three.js/GLB P5 交付，不再以“是否需要真 3D”作为待定项。

多视角内容由 PRD 11 与 11-03 承接；Three.js/GLB 生产路径由 Design 01/20 和最新人物研究统一管理。
...
一手来源是 [LerSent 原帖](https://x.com/LerSentAI/status/2090783821452943404)。原帖只称 Woo 为“立体数字衣橱”，没有称其为实时三维网格，也没有公开技术栈。逐帧证据见 [Woo 参考截图](https://github.com/StephenQiu30/then-server/blob/dd35b002dc6e85c983a5c44a784756bdec3f1af2/docs/design/evidence/woo/README.md)。
...
- 本多视角内容切片不制作三维资产；Three.js/GLB 的独立人物交付继续执行 Design 20。
...
真 3D 已是完整产品的固定能力。进入生产实现前必须关闭以下条件；条件用于选择资产路线和验证可交付性，不再用于决定是否需要真 3D：

1. 有可持续的原创/授权人物和服装生产能力，能处理 topology、rig、skinning、body coverage、LOD、材质和穿模。
2. 资产合同固定人物/服装实体、骨架、槽位、动作、相机锚点、版本、hash 与兼容范围。
3. Three.js/WKWebView/GLB 在最低真机完成画质、内存、帧时间、发热、无障碍与崩溃验证。
4. 换装、连续观察、缩放、展示动作、原子缓存、坏资源恢复和多视角回退通过独立切片验收。
5. 不得把 Hunyuan3D 的静态网格、VTON360 的研究结果或 360° 视频直接当作可换装角色。

在这些条件关闭前，具体资产供应商不进入生产依赖。iOS 运行时只采用 Design 01/20 固定的 Three.js/WKWebView/GLB，项目不使用 Blender，不增加其他 renderer 或格式兼容分支。
