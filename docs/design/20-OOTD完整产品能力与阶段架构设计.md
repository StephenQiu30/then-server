...
本地 `fixture` Provider 不保存远端任务状态；清理时必须同时核对任务持久化的 Provider 和外部任务身份，不能只凭 `fixture:` 字符串判定无远端副本。私有 MinIO 输出仍必须按固定版本实际删除并完成清理记录；非 fixture Provider 身份继续失败关闭，直到对应删除适配器与准入完成。
