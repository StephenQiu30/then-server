...
图片结果通过字节校验后仍需本人明确确认；确认时间与不可变输出分开记录。模型任务接纳必须在同一事务核实图片已确认、未撤权且 owner/Look/revision/hash 匹配。重复确认返回原确认时间；删除或撤权后不能再确认。

任务状态采用 `queued → running → validating → succeeded`，并有 `failed/canceled/expired` 终态；取消请求、外部取消确认和删除清理状态独立保存。返回 202 只表示任务被接纳。供应商提交超时且不确定是否已接纳时，先按外部任务标识或可用查询对账；不能盲目重提造成二次计费。供应商不支持可靠幂等/查询时，管理员可经专用入口按状态 revision 和非敏感证据编号核对受理结果；任务状态、额度结算和审计记录原子提交。确认受理时补录外部任务 ID 并进入既有迟到清理；确认未受理时终结为失败，已取消或撤销任务则结算为 canceled。该流程不自动重提或查询 Provider。已发布结果的最长可读时间从 `published_at` 起计算，过期即停止签发读取凭据和作为模型来源；独立的清理轮询复用删除清单撤权及精确对象版本清理。结果过期不改写已结算的任务成功或额度事实，任务处理超时仍使用任务 `expired` 状态。
...
本地 `fixture` Provider 不保存远端任务状态；清理时必须核对目标 Provider 和外部任务 ID 均与同一任务的持久化事实一致，不能只凭 `fixture:` 字符串判定无远端副本。私有 MinIO 输出仍必须按固定版本实际删除并完成清理记录；身份不符的清理单保持撤权并记录为需处理的失败，不能中断其他清理单；非 fixture Provider 身份继续失败关闭，直到对应删除适配器与准入完成。
