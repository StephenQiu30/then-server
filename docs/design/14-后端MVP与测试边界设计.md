...
| B3 同步与远程目录 | 衣橱/搭配/穿着增量同步、远程资产目录 | 17-20 已实现账号下无图衣橱 CRUD 基础；App 同步未接入 | 冲突、墓碑、版本、离线恢复与多设备产品契约获批 |

B0 不伪造业务 API。B1/B2 只实现 17-19 已批准的本人照片准备子集；B3 已具备 17-20 的账号级无图 CRUD 基础，增量同步、冲突合并、墓碑和多设备恢复仍只保留路线图。
