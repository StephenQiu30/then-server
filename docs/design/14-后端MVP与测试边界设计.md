...
| B2 单次生成闭环 | 上传意图/finalize、任务创建/查询/取消、MinIO 私有资产、PG Outbox/Inbox、Kafka worker | 照片准备与删除已完成；第三方生成任务未实现 | Provider、地域、成本、质量、迟到结果及删除 POC 通过 |
...
- `main.go` 只委托 bootstrap；业务按 `httpapi → application/<feature>` 进入，PostgreSQL/MinIO/Kafka 作为 adapter 由 bootstrap 注入。
...
- 本机 services 测试只允许 loopback；PostgreSQL 使用临时表，MinIO 使用随机桶，Redis 默认 DB 15 与随机 key，Kafka 使用随机 topic prefix 与独立消费组，所有资源由测试清理。

Go 官方建议将服务器实现放入 `internal`，服务命令放入命名的 `cmd/<binary>`，并通过 `_test.go` 与 `go test` 使用内建测试能力；本项目按 2026-09-22 用户要求固定根目录 `main.go` 与 `internal/bootstrap`，不为单一入口保留 `cmd` 包装目录，包内单元测试继续跟随被测 package。依据：[Organizing a Go module](https://go.dev/doc/modules/layout)、[Add a test](https://go.dev/doc/tutorial/add-a-test)。build tag 只隔离真实依赖测试，不用于生产功能分支。
...
| Local services | `backend/tests/services`；`-tags=services` | 本机 PG/MinIO/Redis/Kafka | 实际协议、鉴权、TTL、发送确认/失败重放 与清理 |
| Integration | `backend/tests/integration`；`-tags=integration` | Testcontainers PG/Redis/MinIO/Kafka | 实际 `all` 进程、合成照片全链、断连恢复、错误退出与契约分发 |
