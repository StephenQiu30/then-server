...
状态：approved。2026-09-14 用户明确要求规划后端功能、固定 SOP、按 MVP 开发，并将 test 服务独立。关联 [PRD 17](../prd/17-云端生成与任务管理需求.md#后端mvp与测试服务边界)、[Design 02](02-后端架构.md#后端开发与交付-sop) 与 [17-11 执行计划](../plan/17-11-后端MVP与测试边界执行计划.md)。
...
| B1 云入口基础 | 内部用户身份、目的同意、撤回与删除接纳、首份 versioned SQL | 待云功能进入近期计划 | 用户行为、数据字段、保留/删除和认证方式在独立切片获批 |
