...
| ID | 状态 | 完成判定 |
| --- | --- | --- |
| 11-04-ASSET-01 | in_progress | 当前工程 GLB 无眼部输入；MetaPerson 官方样例已验证眼睑/注视 morph、眼球节点和实际闭眼，Avaturn 官方样例未发现眼部数据。仍须取得可爱卡通、模块衣物、正式授权、价格与体积合格的代表 GLB，采购前确认预算 |
...
候选实测只关闭 `11-04-ASSET-01` 的技术可行性子项。供应商样例未进入 App，且 MetaPerson 当前样例不符合 Woo 可爱画风、授权与资源预算，Avaturn 当前样例不具备眼部输入；因此 `11-04-EYES-01` 仍不能开工。正式代表包必须先通过 ACC-026，避免用节点名映射或兼容代码掩盖资产缺陷。
