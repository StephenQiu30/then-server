...
代表完整 Look 验证 → 单套图片/GLB 与本地保存闭环 → 三人物六套内置内容 → 完整穿搭图和按需模型云任务。云认证、同意、实际账号额度、地域/许可与删除在真实调用前就绪；完整跨设备同步、视频和高级角色分别排期。

用户此前暂停 App 编码，本轮要求替换设计，没有自动恢复 App 实现或批准付费调用。现有前后端目录/组件整理按 17-26～29 的独立授权与计划继续；不重做已完成的账户、衣橱、日记和社区后端。
