...
根目录 `design.md` 是全仓库通用页面设计与交付规范，适用于页面视觉质量、组件选择、排版、响应式、交互表达和设计验收。它不定义于是 OOTD 的产品范围、品牌身份、技术栈、数据模型、隐私边界或功能状态；这些内容继续以本文件和 `docs/` 对应事实源为准。应用 `design.md` 时必须将其中的示例品牌和示例内容替换为于是 OOTD 的真实内容，不得把示例技术或产品声明引入实现。
...
6. 根目录 `design.md` 中适用于通用页面视觉与交付质量的规范。
7. 已批准的对应 `FF-SS` 单切片执行计划。
8. `backend/openapi.yaml` 与已发布数据库 migration 等机器事实源。
9. 当前 OOTD 产品级实施计划与 acceptance。
10. 现有代码和测试体现的行为。
