...
**当前 Web 已实现 `/register`、`/login`、`/account`、`/wardrobe` 和 `/plans`**。账户、无图衣橱与穿搭计划分别依据 [Design 16](docs/design/16-Web账户管理设计.md)、[Design 25](docs/design/25-Web衣橱数据核验设计.md)、[Design 26](docs/design/26-Web穿搭计划数据核验设计.md)；均已完成本地真实服务验证。`/plans` 与后端 `/outfit-plans` 分开，避免同源代理冲突。公开注册门禁仍在 17-13-REL-01；Web 数据页不抵扣 App 离线 Look、推荐或实际穿着验收。
...
| 账户、无图衣橱和计划页面已在真实服务完成本地验证，rewrites 覆盖当前业务路径 | 新增业务域时按切片同步 API 转发与发布条件；页面路径避开同名 API 根路径，公开入口仍须通过账号门禁 |
