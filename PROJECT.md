...
**当前 Web 已实现 `/register`、`/login`、`/account` 和 `/wardrobe`**。账户页面依据 [Web 账户管理设计](docs/design/16-Web账户管理设计.md)，衣橱页依据 [Web 衣橱数据核验设计](docs/design/25-Web衣橱数据核验设计.md)；均已完成本地真实服务验证。公开注册门禁仍在 17-13-REL-01；Web 衣橱不抵扣 App 离线 Look 或推荐验收。其他能力进入各自页面设计和实施切片后再建设。
...
| 账户和无图衣橱页面已在真实服务完成本地验证，rewrites 覆盖当前业务路径 | 新增业务域时按切片同步 API 转发与发布条件；公开入口仍须通过账号门禁 |
