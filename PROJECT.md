...
| 能力 | 必须保留的需求 | 交付边界 |
| --- | --- | --- |
| 默认穿搭 | 三套人物预设、内置服装、换装、保存 Look 与日期；默认 App 路径零账号、零上传、离线可用 | Web 不因采用同一设计体系而自动承诺离线/PWA 或完整 App 功能 |
...
| AI 与三维 | 本人试穿、Create 360°、Three.js/GLB 交互三维属于完整目标 | 分阶段验收；失败不阻塞衣橱、保存与记录；保留禁止 Blender 的既定约束 |
