...
**当前 Web 首个业务切片固定为 `/register`、`/login`、`/account`**，依据 [Web 账户管理设计](docs/design/16-Web账户管理设计.md)。三个页面已完成本地开发验证；公开注册门禁仍在 17-13-REL-01。其他能力进入各自页面设计和实施切片后再建设，不提前建立占位路由。
...
| 账户页面与真实服务旅程已本地验证，rewrites 覆盖当前账户路径 | 新增业务域时按切片同步 API 转发与发布条件 |
