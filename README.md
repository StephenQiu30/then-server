...
采用“二维衣橱与选衣 → 完整穿搭图 → 按需整套静态 GLB → Three.js 查看与缓存复用”。内置完整 Look 零账号、零上传、离线可用；首版不要求独立三维衣物、骨架或眼部动作。设计已于 2026-09-22 按用户要求替换，正式资产与生成链路尚未实现，详见 [完整设计](docs/design/20-OOTD完整产品能力与阶段架构设计.md)、[离线闭环计划](docs/plan/11-05-三维人物与造型闭环执行计划.md) 和 [云生成计划](docs/plan/14-01-完整穿搭图与按需三维生成执行计划.md)。

## 当前实现

Go + Gin/Huma + GORM/AutoMigrate + PostgreSQL 已实现健康检查、注册/登录、Cookie 会话、本人账户 CRUD、本人成年声明、结构化衣橱、账号穿搭计划、实际穿着、私人日记、服务端增量同步，以及社区帖子审核治理。运行时 OpenAPI 是 Swagger 与 Umi 请求生成的唯一输入。目录与规范见 [后端架构](docs/design/02-后端架构.md)，精确版本见 [技术选型](docs/design/01-技术选型.md)。

日常使用本机已安装 PostgreSQL、Redis、Kafka、MinIO；当前 API 以 PostgreSQL 保存账号、会话、声明、结构化衣橱、计划、实际穿着、私人日记、增量变更、社区审核治理和媒体事实，Redis 只保存认证限流短期计数。Kafka 与 MinIO 已用于显式开启的私有图片开发闭环，默认仍关闭。`frontend` 已建立 Next.js App Router/TypeScript、shadcn/ui + Radix 基础组件、统一设计 token 与运行时 OpenAPI 生成客户端，业务页面尚未实现；App 云接入、生成和主动同步未完成。
...
GitHub Actions 的 `Go quality`、`PostgreSQL integration` 与 `OCI container` 是每次 push/PR 的必要服务端门禁。需要本机 MinIO、Redis 和 Kafka 的 `services` 测试仍按对应验收显式运行。
