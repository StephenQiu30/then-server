	...
	case address := <-started:
		...
		for _, path := range []string{"/docs/", "/docs/swagger-ui-bundle.js", "/openapi.yaml", "/openapi.json", "/health/ready"} {
			...
			if path == "/openapi.json" {
				...
				if json.Unmarshal(body, &contract) != nil || contract.OpenAPI != "3.1.2" || contract.Info.Version != "0.29.0" || !strings.Contains(string(body), `"operationId":"listSyncChanges"`) || !strings.Contains(string(body), `"operationId":"createWearEvent"`) || !strings.Contains(string(body), `"operationId":"createDiaryEntry"`) || !strings.Contains(string(body), `"operationId":"decidePostModeration"`) || !strings.Contains(string(body), `"operationId":"listCommunityFeed"`) || !strings.Contains(string(body), `"operationId":"requestEmailVerification"`) || !strings.Contains(string(body), `"operationId":"confirmPasswordReset"`) || !strings.Contains(string(body), `"operationId":"createDataExport"`) || !strings.Contains(string(body), `"operationId":"getAccountDeletionReceipt"`) || !strings.Contains(string(body), `"operationId":"getPublicProfileAvatar"`) || !strings.Contains(string(body), `"operationId":"listCurrentUserSessions"`) || !strings.Contains(string(body), `"operationId":"revokeCurrentUserSession"`) || !strings.Contains(string(body), `"operationId":"archiveWardrobeItem"`) || !strings.Contains(string(body), `"operationId":"restoreWardrobeItem"`) || !strings.Contains(string(body), `"operationId":"getGenerationOutputAccess"`) || !strings.Contains(string(body), `"operationId":"listUnknownGenerationSubmissions"`) || !strings.Contains(string(body), `"operationId":"reconcileUnknownGenerationSubmission"`) {
