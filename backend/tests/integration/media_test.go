...
func TestActualBinarySyntheticMediaLifecycle(t *testing.T) {
	...
	minioContainer := integrationContainer(t, ctx, testcontainers.ContainerRequest{Image: testcontainer.MinIOImage(), Cmd: []string{"server", "/data"}, Env: map[string]string{"MINIO_ROOT_USER": "then_test", "MINIO_ROOT_PASSWORD": minioPassword}, ExposedPorts: []string{"9000/tcp"}, WaitingFor: wait.ForHTTP("/minio/health/live").WithPort("9000/tcp").WithStartupTimeout(time.Minute)})
