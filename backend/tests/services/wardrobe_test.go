...
func TestWardrobePersistenceLifecycle(t *testing.T) {
	...
	first, err := accounts.Register(ctx, domain.RegisterAccountInput{Email: "wardrobe-first@example.test", DisplayName: "First", Password: "correct-password-first"})
	serviceOK(t, "register first wardrobe owner", err)
	second, err := accounts.Register(ctx, domain.RegisterAccountInput{Email: "wardrobe-second@example.test", DisplayName: "Second", Password: "correct-password-second"})
	...
	input := domain.CreateWardrobeItemInput{ID: sharedID, Name: "Blue Shirt", Category: domain.WardrobeTop, Availability: domain.WardrobeWearable, Source: domain.WardrobeSourceWardrobe, Attributes: domain.WardrobeAttributes{FormalityBand: wardrobeTestValue(domain.WardrobeFormalitySmartCasual), WarmthBand: wardrobeTestValue(domain.WardrobeWarmthLight), RainUse: wardrobeTestValue(domain.WardrobeUseSuitable)}}
	created, err := wardrobe.CreateWardrobeItem(ctx, first.Token, input)
	serviceOK(t, "create first wardrobe item", err)
	if created.Attributes.FormalityBand == nil || *created.Attributes.FormalityBand != domain.WardrobeFormalitySmartCasual || created.Attributes.WalkingUse != nil {
	...
	conflict := input
	conflict.Attributes.RainUse = wardrobeTestValue(domain.WardrobeUseUnsuitable)
	if _, err := wardrobe.CreateWardrobeItem(ctx, first.Token, conflict); !errors.Is(err, domain.ErrWardrobeConflict) {
	...
	private, err := wardrobe.CreateWardrobeItem(ctx, first.Token, domain.CreateWardrobeItemInput{ID: privateID, Name: "Black Shoes", Category: domain.WardrobeShoes, Availability: domain.WardrobePacked, Source: domain.WardrobeSourceQuickAdd})
	serviceOK(t, "create owner-only wardrobe item", err)
	thirdID := "018f1f74-a2d0-7c6d-9c17-4a0ea2400b13"
	_, err = wardrobe.CreateWardrobeItem(ctx, first.Token, domain.CreateWardrobeItemInput{ID: thirdID, Name: "Rain Coat", Category: domain.WardrobeOuterwear, Availability: domain.WardrobeLaundry, Source: domain.WardrobeSourceWardrobe})
	serviceOK(t, "create pagination wardrobe item", err)
	if _, err := wardrobe.GetWardrobeItem(ctx, second.Token, privateID); !errors.Is(err, domain.ErrWardrobeNotFound) {
	...
	if _, err := wardrobe.ListWardrobeItems(ctx, second.Token, 2, firstPage.NextAfterID); !errors.Is(err, domain.ErrWardrobeNotFound) {
		t.Fatal("cross-owner wardrobe cursor leaked a different result")
	}

	updated, err := wardrobe.UpdateWardrobeItem(ctx, first.Token, private.ID, private.Revision, domain.UpdateWardrobeItemInput{Name: "Black Walking Shoes", Category: domain.WardrobeShoes, Availability: domain.WardrobeWearable, Attributes: domain.WardrobeAttributes{WarmthBand: wardrobeTestValue(domain.WardrobeWarmthMedium), WalkingUse: wardrobeTestValue(domain.WardrobeUseSuitable)}})
	serviceOK(t, "update current wardrobe revision", err)
	if updated.Revision != private.Revision+1 || updated.Source != domain.WardrobeSourceQuickAdd || updated.Name != "Black Walking Shoes" || updated.Attributes.FormalityBand != nil || updated.Attributes.WarmthBand == nil || *updated.Attributes.WarmthBand != domain.WardrobeWarmthMedium {
	...
	if reloaded.Attributes.WalkingUse == nil || *reloaded.Attributes.WalkingUse != domain.WardrobeUseSuitable || reloaded.Attributes.RainUse != nil {
		t.Fatal("wardrobe attributes did not survive a fresh repository read")
	}
	if _, err := wardrobe.UpdateWardrobeItem(ctx, first.Token, private.ID, private.Revision, domain.UpdateWardrobeItemInput{Name: "Stale", Category: domain.WardrobeShoes, Availability: domain.WardrobeLaundry}); !errors.Is(err, domain.ErrWardrobeConflict) {
	...
	if err := wardrobe.DeleteWardrobeItem(ctx, first.Token, private.ID, private.Revision, domain.WardrobeHistoryRedactSnapshots, impact.ExpectedImpact); !errors.Is(err, domain.ErrWardrobeConflict) {
		t.Fatal("stale wardrobe delete was accepted")
	}
	serviceOK(t, "delete current wardrobe revision", wardrobe.DeleteWardrobeItem(ctx, first.Token, private.ID, updated.Revision, domain.WardrobeHistoryRedactSnapshots, impact.ExpectedImpact))
	if _, err := wardrobe.GetWardrobeItem(ctx, first.Token, private.ID); !errors.Is(err, domain.ErrWardrobeNotFound) {
