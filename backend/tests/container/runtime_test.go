...
func TestContainerRuntime(t *testing.T) {
	...
	t.Run("failed startup cleanup", func(t *testing.T) {
		var err error
		failed, err = testcontainer.Create(t, ctx, testcontainers.GenericContainerRequest{
	...
	password := rand.Text()
	db, err := testcontainer.Create(t, ctx, testcontainers.GenericContainerRequest{
		ContainerRequest: testcontainers.ContainerRequest{
	...
	api, err := testcontainer.Create(t, ctx, testcontainers.GenericContainerRequest{
		ContainerRequest: testcontainers.ContainerRequest{
			Image: image,
	...
	for _, role := range []string{"worker", "all"} {
		t.Run(role+" restricted runtime", func(t *testing.T) {
			...
			runtime, err := testcontainer.Create(t, ctx, testcontainers.GenericContainerRequest{ContainerRequest: testcontainers.ContainerRequest{
