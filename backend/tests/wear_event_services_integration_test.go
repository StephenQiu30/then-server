...
func TestWearEventPersistenceLifecycle(t *testing.T) {
	...
	serviceOK(t, "migrate wear event schema", repository.Migrate(ctx, database))
	...
	owner, err := accounts.Register(ctx, model.RegisterAccountInput{Email: "wear-owner@example.test", DisplayName: "Owner", Password: "correct-password-owner"})
	serviceOK(t, "register wear event owner", err)
	other, err := accounts.Register(ctx, model.RegisterAccountInput{Email: "wear-other@example.test", DisplayName: "Other", Password: "correct-password-other"})
	...
	wear, err := service.NewWearEventService(accounts, repository.NewWearEventRepository(database))
	serviceOK(t, "construct wear event service", err)

	shirt, err := wardrobe.CreateWardrobeItem(ctx, owner.Token, model.CreateWardrobeItemInput{ID: "018f1f74-a2d0-7c6d-9c17-4a0ea2400f01", Name: "Blue Shirt", Category: model.WardrobeTop, Availability: model.WardrobeWearable, Source: model.WardrobeSourceWardrobe})
	...
	plan, err := outfits.CreateOutfitPlan(ctx, owner.Token, planID, model.OutfitPlanInput{LocalDate: today, TimeZone: "Asia/Shanghai", Items: []model.OutfitSelection{{ItemID: shirt.ID, Revision: shirt.Revision}}})
	...
	if notWorn.Status != model.OutfitPlanNotWorn || notWorn.Revision != plan.Revision+1 {
	...
	if plan.Status != model.OutfitPlanActive || plan.Revision != notWorn.Revision+1 {
	...
	input := model.WearEventInput{LocalDate: today, TimeZone: "Asia/Shanghai", Completeness: model.WearEventComplete, Items: []model.OutfitSelection{{ItemID: shirt.ID, Revision: shirt.Revision}}, LaundryItemIDs: []string{shirt.ID}, SourcePlanID: &planID, SourcePlanRevision: &sourceRevision, SourceKind: model.WearEventFollowedPlan}
	...
	if _, err := wear.CreateWearEvent(ctx, owner.Token, eventID, changedCommand); !errors.Is(err, model.ErrWearEventConflict) {
		t.Fatal("same wear event ID accepted a create command with different side effects")
	}
	if _, err := wear.GetWearEvent(ctx, other.Token, eventID); !errors.Is(err, model.ErrWearEventNotFound) {
	...
	if completed.Status != model.OutfitPlanCompleted || completed.Revision != plan.Revision+1 {
	...
	if currentShirt.Availability != model.WardrobeLaundry || currentShirt.Revision != shirt.Revision+1 {
		t.Fatal("wear event laundry selection was not committed atomically")
	}

	secondID := "018f1f74-a2d0-7c6d-9c17-4a0ea2400f04"
	secondInput := model.WearEventInput{LocalDate: today, TimeZone: "Asia/Shanghai", Completeness: model.WearEventPartial, Items: []model.OutfitSelection{{ItemID: currentShirt.ID, Revision: currentShirt.Revision}}, ConfirmedUnavailableIDs: []string{currentShirt.ID}, SourceKind: model.WearEventUnplanned}
	if _, err := wear.CreateWearEvent(ctx, owner.Token, secondID, secondInput); !errors.Is(err, model.ErrWearEventDuplicate) {
		t.Fatal("similar same-day wear event did not require explicit confirmation")
	} else {
		var duplicate *model.WearEventDuplicateError
	...
	if activeAgain.Status != model.OutfitPlanActive || activeAgain.Revision != completed.Revision+1 {
		t.Fatal("deleting the last linked wear event did not restore the plan")
	}
	if _, err := wear.CreateWearEvent(ctx, owner.Token, eventID, input); !errors.Is(err, model.ErrWearEventConflict) {
	...
	serviceOK(t, "redact wardrobe history", wardrobe.DeleteWardrobeItem(ctx, owner.Token, currentShirt.ID, currentShirt.Revision, model.WardrobeHistoryRedactSnapshots, impact.ExpectedImpact))
