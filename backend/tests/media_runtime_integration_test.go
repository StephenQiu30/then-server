...
func TestActualBinarySyntheticMediaLifecycle(t *testing.T) {
	...
	minioContainer := integrationContainer(t, ctx, testcontainers.ContainerRequest{Image: "minio/minio:RELEASE.2025-09-07T16-13-09Z@sha256:14cea493d9a34af32f524e538b8346cf79f3321eff8e708c1e2960462bd8936e", Cmd: []string{"server", "/data"}, Env: map[string]string{"MINIO_ROOT_USER": "then_test", "MINIO_ROOT_PASSWORD": minioPassword}, ExposedPorts: []string{"9000/tcp"}, WaitingFor: wait.ForHTTP("/minio/health/live").WithPort("9000/tcp").WithStartupTimeout(time.Minute)})
