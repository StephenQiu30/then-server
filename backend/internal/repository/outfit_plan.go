...
func (r *OutfitPlanRepository) CreateOutfitPlan(ctx context.Context, ownerID, planID string, input model.OutfitPlanInput, at time.Time) (model.OutfitPlan, error) {
	...
	err := r.database.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if !errors.Is(err, model.ErrOutfitPlanNotFound) {
			return err
		}
		var deleted int64
		...
		record := outfitPlanRecord{OwnerID: ownerID, ID: planID, LocalDate: date, TimeZone: input.TimeZone, ContextSummary: input.ContextSummary, Status: string(model.OutfitPlanActive), Revision: 1, CreatedAt: at, UpdatedAt: at}
...
func (r *OutfitPlanRepository) ListOutfitPlans(ctx context.Context, ownerID string, limit int, afterID, localDate *string) (model.OutfitPlanPage, error) {
	...
	if afterID != nil {
		...
		if err := cursorQuery.First(&cursor).Error; err != nil {
			return model.OutfitPlanPage{}, outfitPlanLookupError(err)
		}
	...
	if err := query.Order("local_date DESC").Order("created_at DESC").Order("id ASC").Limit(limit + 1).Find(&records).Error; err != nil {
		return model.OutfitPlanPage{}, model.ErrOutfitPlanUnavailable
	}
	...
	page := model.OutfitPlanPage{Plans: make([]model.OutfitPlan, 0, len(records))}
...
func (r *OutfitPlanRepository) GetOutfitPlan(ctx context.Context, ownerID, planID string) (model.OutfitPlan, error) {
	plan, err := readOutfitPlan(r.database.WithContext(ctx), ownerID, planID)
	if err != nil {
		return model.OutfitPlan{}, outfitPlanLookupError(err)
	}
	return plan, nil
}

func (r *OutfitPlanRepository) UpdateOutfitPlan(ctx context.Context, ownerID, planID string, expectedRevision int, input model.OutfitPlanInput, at time.Time) (model.OutfitPlan, error) {
	var result model.OutfitPlan
	err := r.database.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if record.Revision != expectedRevision || record.Status != string(model.OutfitPlanActive) || record.TimeZone != input.TimeZone {
			return model.ErrOutfitPlanConflict
		}
		date, _ := time.Parse("2006-01-02", input.LocalDate)
...
func (r *OutfitPlanRepository) CancelOutfitPlan(ctx context.Context, ownerID, planID string, expectedRevision int, at time.Time) (model.OutfitPlan, error) {
	var result model.OutfitPlan
	err := r.database.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var record outfitPlanRecord
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("owner_id = ? AND id = ?", ownerID, planID).First(&record).Error; err != nil {
		...
		if record.Revision != expectedRevision || record.Status != string(model.OutfitPlanActive) {
			return model.ErrOutfitPlanConflict
		}
		if at.Before(record.UpdatedAt) {
			at = record.UpdatedAt
		}
		record.Status, record.Revision, record.UpdatedAt = string(model.OutfitPlanCancelled), record.Revision+1, at
...
func snapshotOutfitItems(tx *gorm.DB, ownerID, planID string, input model.OutfitPlanInput) ([]outfitPlanItemRecord, error) {
	...
	if len(wardrobe) != len(ids) {
		return nil, model.ErrOutfitPlanConflict
	}
	...
	for ordinal, selected := range input.Items {
		...
		if unavailable && !confirmed[item.ID] {
			return nil, model.ErrOutfitItemsUnavailable
		}
		if !unavailable && confirmed[item.ID] {
			return nil, model.ErrOutfitPlanConflict
		}
...
func readOutfitPlan(tx *gorm.DB, ownerID, planID string) (model.OutfitPlan, error) {
	var record outfitPlanRecord
	...
	itemsByPlan, err := readOutfitItemsForPlans(tx, ownerID, []outfitPlanRecord{record})
	if err != nil {
		return model.OutfitPlan{}, model.ErrOutfitPlanUnavailable
	}
...
func outfitFromRecord(record outfitPlanRecord, items []outfitPlanItemRecord) model.OutfitPlan {
	result := model.OutfitPlan{OwnerID: record.OwnerID, ID: record.ID, LocalDate: record.LocalDate.Format("2006-01-02"), TimeZone: record.TimeZone, ContextSummary: copyString(record.ContextSummary), Status: model.OutfitPlanStatus(record.Status), Revision: record.Revision, CreatedAt: record.CreatedAt, UpdatedAt: record.UpdatedAt, Items: make([]model.OutfitPlanItemSnapshot, 0, len(items))}
	for _, item := range items {
...
func outfitPlanMatchesInput(plan model.OutfitPlan, input model.OutfitPlanInput) bool {
	if plan.Status != model.OutfitPlanActive || plan.LocalDate != input.LocalDate || plan.TimeZone != input.TimeZone || !stringsEqual(plan.ContextSummary, input.ContextSummary) || len(plan.Items) != len(input.Items) {
		return false
	}
	confirmed := make(map[string]bool, len(input.ConfirmedUnavailableIDs))
	for _, itemID := range input.ConfirmedUnavailableIDs {
	...
	for index, selected := range input.Items {
		...
		if content == nil || content.ItemID != selected.ItemID || content.ItemRevision != selected.Revision || confirmed[selected.ItemID] != (content.Availability != model.WardrobeWearable) {
			return false
		}
	}
	return true
...
func outfitPlanLookupError(err error) error {
	if errors.Is(err, model.ErrOutfitPlanNotFound) || errors.Is(err, gorm.ErrRecordNotFound) {
		return model.ErrOutfitPlanNotFound
	}
	return model.ErrOutfitPlanUnavailable
}
...
func outfitPlanWriteError(err error) error {
	...
	for _, known := range []error{model.ErrInvalidOutfitPlanInput, model.ErrOutfitPlanNotFound, model.ErrOutfitPlanConflict, model.ErrOutfitItemsUnavailable} {
