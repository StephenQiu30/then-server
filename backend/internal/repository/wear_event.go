...
func (r *WearEventRepository) CreateWearEvent(ctx context.Context, ownerID, eventID string, input model.WearEventInput, at time.Time) (model.WearEvent, error) {
	var result model.WearEvent
	...
	err := r.database.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err == nil {
			if existingRecord.CreateFingerprint != fingerprint {
				return model.ErrWearEventConflict
			}
	...
	if err != nil {
		return model.WearEvent{}, wearEventWriteError(err)
	}
	return result, nil
}

func (r *WearEventRepository) ListWearEvents(ctx context.Context, ownerID string, limit int, afterID, localDate *string) (model.WearEventPage, error) {
	...
	if afterID != nil {
		...
		if err := cursorQuery.First(&cursor).Error; err != nil {
			return model.WearEventPage{}, wearEventLookupError(err)
		}
	...
	if err := query.Order("local_date DESC").Order("created_at DESC").Order("id ASC").Limit(limit + 1).Find(&records).Error; err != nil {
		return model.WearEventPage{}, model.ErrWearEventServiceUnavailable
	}
	...
	page := model.WearEventPage{Events: make([]model.WearEvent, 0, len(records))}
...
func (r *WearEventRepository) GetWearEvent(ctx context.Context, ownerID, eventID string) (model.WearEvent, error) {
	return readWearEvent(r.database.WithContext(ctx), ownerID, eventID)
}

func (r *WearEventRepository) UpdateWearEvent(ctx context.Context, ownerID, eventID string, expectedRevision int, input model.WearEventInput, at time.Time) (model.WearEvent, error) {
	var result model.WearEvent
	err := r.database.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func writeWearEvent(tx *gorm.DB, ownerID, eventID string, old *model.WearEvent, input model.WearEventInput, createFingerprint string, at time.Time) (model.WearEvent, error) {
	...
	if !sameWearCandidates(candidates, input.DuplicateConfirmations) {
		return model.WearEvent{}, &model.WearEventDuplicateError{Candidates: candidates}
	}
	...
	if old == nil {
		...
		if created.Error != nil {
			return model.WearEvent{}, created.Error
		}
		if created.RowsAffected == 0 {
			...
			if err := tx.Where("owner_id = ? AND id = ?", ownerID, eventID).First(&existing).Error; err != nil || existing.CreateFingerprint != createFingerprint {
				return model.WearEvent{}, model.ErrWearEventConflict
			}
		...
		if updated.Error != nil {
			return model.WearEvent{}, updated.Error
		}
...
func validateWearSourcePlan(tx *gorm.DB, ownerID string, old *model.WearEvent, input model.WearEventInput) error {
	if input.SourcePlanID == nil {
		return nil
	}
	...
	if plan.Status != string(model.OutfitPlanActive) && plan.Status != string(model.OutfitPlanCompleted) {
		return model.ErrWearEventConflict
	}
	return nil
}

func snapshotWearItems(tx *gorm.DB, ownerID, eventID string, input model.WearEventInput) ([]wearEventItemRecord, map[string]wardrobeItemRecord, error) {
	...
	if len(records) != len(ids) {
		return nil, nil, model.ErrWearEventConflict
	}
	...
	for ordinal, selected := range input.Items {
		...
		unavailable := item.Availability != string(model.WardrobeWearable)
		if unavailable != confirmed[item.ID] {
			if unavailable {
				return nil, nil, model.ErrWearEventItemsUnavailable
			}
			return nil, nil, model.ErrWearEventConflict
		}
...
func applyLaundrySelection(tx *gorm.DB, wardrobe map[string]wardrobeItemRecord, selected []string, at time.Time) error {
	for _, itemID := range selected {
		...
		if item.Availability == string(model.WardrobeLaundry) {
			continue
		}
		updatedAt := at
		...
		updated := tx.Model(&wardrobeItemRecord{}).Where("owner_id = ? AND id = ? AND revision = ?", item.OwnerID, item.ID, item.Revision).Updates(map[string]any{"availability": string(model.WardrobeLaundry), "revision": item.Revision + 1, "updated_at": updatedAt})
		if updated.Error != nil {
...
func duplicateWearEvents(tx *gorm.DB, ownerID, eventID, localDate string, selected []model.OutfitSelection) ([]model.WearEventCandidate, error) {
	var records []wearEventRecord
	...
	result := []model.WearEventCandidate{}
	for _, record := range records {
		...
		if union > 0 && float64(intersection)/float64(union) >= 0.8 {
			result = append(result, model.WearEventCandidate{ID: record.ID, Revision: record.Revision})
		}
	}
	return result, nil
}
...
func recomputeOutfitPlanStatus(tx *gorm.DB, ownerID, planID string, at time.Time) error {
	...
	if plan.Status == string(model.OutfitPlanCancelled) || plan.Status == string(model.OutfitPlanNotWorn) {
		return nil
	}
	var count int64
	...
	desired := string(model.OutfitPlanActive)
	if count > 0 {
		desired = string(model.OutfitPlanCompleted)
	}
...
func readWearEvent(tx *gorm.DB, ownerID, eventID string) (model.WearEvent, error) {
	var record wearEventRecord
	if err := tx.Where("owner_id = ? AND id = ?", ownerID, eventID).First(&record).Error; err != nil {
		return model.WearEvent{}, wearEventLookupError(err)
	}
	items, err := readWearItemsForEvents(tx, ownerID, []wearEventRecord{record})
	if err != nil {
		return model.WearEvent{}, model.ErrWearEventServiceUnavailable
	}
...
func wearEventFromRecord(record wearEventRecord, items []wearEventItemRecord) model.WearEvent {
	result := model.WearEvent{ID: record.ID, OwnerID: record.OwnerID, LocalDate: record.LocalDate.Format("2006-01-02"), TimeZone: record.TimeZone, Completeness: model.WearEventCompleteness(record.Completeness), ContextSummary: copyString(record.ContextSummary), SourcePlanID: copyString(record.SourcePlanID), SourcePlanRevision: copyInt(record.SourcePlanRevision), SourceKind: model.WearEventSourceKind(record.SourceKind), Revision: record.Revision, CreatedAt: record.CreatedAt, UpdatedAt: record.UpdatedAt, Items: make([]model.OutfitPlanItemSnapshot, 0, len(items))}
	for _, item := range items {
		snapshot := model.OutfitPlanItemSnapshot{Ordinal: item.Ordinal}
		if !item.Redacted && item.WardrobeItemID != nil && item.ItemRevision != nil && item.Name != nil && item.Category != nil && item.Availability != nil {
			snapshot.Content = &model.OutfitItemContent{ItemID: *item.WardrobeItemID, ItemRevision: *item.ItemRevision, Name: *item.Name, Category: model.WardrobeCategory(*item.Category), Availability: model.WardrobeAvailability(*item.Availability), Attributes: model.WardrobeAttributes{FormalityBand: typedPointer[model.WardrobeFormalityBand](item.FormalityBand), WarmthBand: typedPointer[model.WardrobeWarmthBand](item.WarmthBand), RainUse: typedPointer[model.WardrobeUseSuitability](item.RainUse), WalkingUse: typedPointer[model.WardrobeUseSuitability](item.WalkingUse)}}
		}
...
func wearEventCreateFingerprint(input model.WearEventInput) string {
	payload := struct {
		LocalDate               string
		TimeZone                string
		Completeness            model.WearEventCompleteness
		ContextSummary          *string
		Items                   []model.OutfitSelection
		LaundryItemIDs          []string
		ConfirmedUnavailableIDs []string
		SourcePlanID            *string
		SourcePlanRevision      *int
		SourceKind              model.WearEventSourceKind
		DuplicateConfirmations  []model.WearEventCandidate
	}{
		...
		DuplicateConfirmations:  append([]model.WearEventCandidate(nil), input.DuplicateConfirmations...),
	}
...
func sameWearCandidates(left, right []model.WearEventCandidate) bool {
	if len(left) != len(right) {
		return false
	}
	l, r := append([]model.WearEventCandidate(nil), left...), append([]model.WearEventCandidate(nil), right...)
...
func wearEventLookupError(err error) error {
	if errors.Is(err, model.ErrWearEventNotFound) || errors.Is(err, gorm.ErrRecordNotFound) {
		return model.ErrWearEventNotFound
	}
	return model.ErrWearEventServiceUnavailable
}
...
func wearEventWriteError(err error) error {
	...
	for _, known := range []error{model.ErrInvalidWearEventInput, model.ErrWearEventNotFound, model.ErrWearEventConflict, model.ErrWearEventDuplicate, model.ErrWearEventItemsUnavailable} {
