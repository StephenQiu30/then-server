...
func registerWardrobeOperations(api huma.API, handler *WardrobeHandler) {
	...
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "recommendWardrobeOutfits", Method: http.MethodPost, Path: "/wardrobe/recommendations", Tags: []string{"Wardrobe"}, Summary: "按本人真实可用衣橱和确认约束推荐穿搭", MaxBodyBytes: 4 * 1024, Errors: wardrobeErrors(http.StatusBadRequest)}), handler.recommend)
	...
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "archiveWardrobeItem", Method: http.MethodPost, Path: "/wardrobe/items/{item_id}/archive", Tags: []string{"Wardrobe"}, Summary: "按 revision 归档本人衣物", Errors: wardrobeErrors(http.StatusBadRequest, http.StatusNotFound, http.StatusConflict)}), handler.archive)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "restoreWardrobeItem", Method: http.MethodPost, Path: "/wardrobe/items/{item_id}/restore", Tags: []string{"Wardrobe"}, Summary: "按 revision 恢复本人衣物", Errors: wardrobeErrors(http.StatusBadRequest, http.StatusNotFound, http.StatusConflict)}), handler.restore)
