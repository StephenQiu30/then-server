...
type AccountService interface {
	Register(context.Context, domain.RegisterAccountInput) (domain.AuthenticatedUser, error)
	Login(context.Context, domain.CreateSessionInput) (domain.AuthenticatedUser, error)
	CurrentUser(context.Context, string) (domain.User, error)
	UpdateCurrentUser(context.Context, string, domain.UpdateCurrentUserInput) (domain.User, error)
	CurrentProfile(context.Context, string) (domain.PublicProfile, error)
	PublicProfile(context.Context, string) (domain.PublicProfile, error)
	PutCurrentProfile(context.Context, string, domain.PutProfileInput) (domain.PublicProfile, error)
...
func (h *AccountHandler) register(ctx context.Context, input *registerAccountInput) (*authenticatedUserOutput, error) {
	...
	result, err := h.service.Register(ctx, domain.RegisterAccountInput{Email: input.Body.Email, DisplayName: input.Body.DisplayName, Password: input.Body.Password})
	if err != nil {
...
func (h *AccountHandler) login(ctx context.Context, input *createSessionInput) (*authenticatedUserOutput, error) {
	...
	result, err := h.service.Login(ctx, domain.CreateSessionInput{Email: input.Body.Email, Password: input.Body.Password})
	if err != nil {
		return nil, accountError(ctx, err)
	}
	return &authenticatedUserOutput{RequestID: requestID(ctx), SetCookie: h.sessionCookie(result.Token, result.ExpiresAt), Body: AuthenticatedUserResponse{User: newUserResponse(result.User)}}, nil
...
func (h *AccountHandler) update(ctx context.Context, input *updateCurrentUserInput) (*userOutput, error) {
	...
	user, err := h.service.UpdateCurrentUser(ctx, input.Session, domain.UpdateCurrentUserInput{
...
func accountError(ctx context.Context, err error) error {
	switch {
	case errors.Is(err, domain.ErrInvalidAccountInput):
	...
	case errors.Is(err, domain.ErrAccountConflict):
		...
		return response
	case errors.Is(err, domain.ErrAccountMediaConflict):
		response := newErrorResponse(http.StatusConflict, requestID(ctx))
