...
func registerGenerationOperations(api huma.API, handler *GenerationHandler) {
	...
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "getGenerationOutputAccess", Method: http.MethodGet, Path: "/generation-jobs/{job_id}/output-access", Tags: []string{"Generation"}, Summary: "获取本人已完成产物的短时读取凭据", Errors: generationErrors(http.StatusBadRequest, http.StatusNotFound)}), handler.outputAccess)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "confirmGenerationImage", Method: http.MethodPost, Path: "/generation-jobs/{job_id}/confirm-image", Tags: []string{"Generation"}, Summary: "确认本人已完成的完整穿搭图", Errors: generationErrors(http.StatusBadRequest, http.StatusConflict, http.StatusNotFound)}), handler.confirmImage)
