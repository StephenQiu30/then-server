...
type MediaService interface {
	CreateConsent(context.Context, string, model.CreateConsentInput) (model.ConsentRecord, error)
	GetConsent(context.Context, string, string) (model.ConsentRecord, error)
	WithdrawConsent(context.Context, string, string) (model.ConsentRecord, error)
	CreateMediaUpload(context.Context, string, model.CreateMediaUploadInput) (model.MediaUpload, error)
	CompleteMediaUpload(context.Context, string, string, model.CompleteMediaUploadInput) (model.MediaAsset, error)
	GetMedia(context.Context, string, string) (model.MediaAsset, error)
	DeleteMedia(context.Context, string, string) (model.DeletionRequest, error)
...
type ConsentResponse struct {
	...
	Status            model.ConsentStatus `json:"status" enum:"active,withdrawn"`
...
type MediaResponse struct {
	...
	ContentType string            `json:"content_type" enum:"image/jpeg"`
	ByteSize    int64             `json:"byte_size" minimum:"1"`
	Status      model.MediaStatus `json:"status" enum:"pending_upload,uploaded,checking,ready,rejected,deleting,deleted"`
...
type DeletionRequestResponse struct {
	...
	Status          model.DeletionStatus `json:"status" enum:"pending,running,complete,failed"`
...
func registerMediaOperations(api huma.API, handler *MediaHandler) {
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "createConsent", Method: http.MethodPost, Path: "/v1/consents", Tags: []string{"Private media"}, Summary: "同意本人照片的固定本地开发用途", DefaultStatus: http.StatusCreated, MaxBodyBytes: 8 * 1024, Errors: mediaErrors(http.StatusBadRequest)}), handler.createConsent)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "getConsent", Method: http.MethodGet, Path: "/v1/consents/{consent_id}", Tags: []string{"Private media"}, Summary: "获取本人同意状态", Errors: mediaErrors(http.StatusNotFound)}), handler.getConsent)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "withdrawConsent", Method: http.MethodPost, Path: "/v1/consents/{consent_id}/withdraw", Tags: []string{"Private media"}, Summary: "撤回本人照片同意", Errors: mediaErrors(http.StatusNotFound)}), handler.withdrawConsent)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "createMediaUpload", Method: http.MethodPost, Path: "/v1/media/uploads", Tags: []string{"Private media"}, Summary: "创建一次 JPEG 私有直传意图", DefaultStatus: http.StatusCreated, MaxBodyBytes: 8 * 1024, Errors: mediaErrors(http.StatusBadRequest, http.StatusConflict, http.StatusRequestEntityTooLarge)}), handler.createUpload)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "completeMediaUpload", Method: http.MethodPost, Path: "/v1/media/{media_id}/complete", Tags: []string{"Private media"}, Summary: "固定已上传对象版本", MaxBodyBytes: 4 * 1024, Errors: mediaErrors(http.StatusBadRequest, http.StatusNotFound, http.StatusConflict)}), handler.completeUpload)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "getMedia", Method: http.MethodGet, Path: "/v1/media/{media_id}", Tags: []string{"Private media"}, Summary: "获取本人媒体处理状态", Errors: mediaErrors(http.StatusNotFound)}), handler.getMedia)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "deleteMedia", Method: http.MethodDelete, Path: "/v1/media/{media_id}", Tags: []string{"Private media"}, Summary: "立即撤销读取并请求删除媒体", DefaultStatus: http.StatusAccepted, Errors: mediaErrors(http.StatusNotFound, http.StatusConflict)}), handler.deleteMedia)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "getDeletionRequest", Method: http.MethodGet, Path: "/v1/deletion-requests/{request_id}", Tags: []string{"Private media"}, Summary: "获取本人媒体删除进度", Errors: mediaErrors(http.StatusNotFound)}), handler.getDeletion)
}
...
func (h *MediaHandler) createConsent(ctx context.Context, input *createConsentInput) (*consentOutput, error) {
	...
	r := input.Body
	if r.Processor != model.MediaProcessorThen || r.Region != model.MediaRegionLocalDevelopment || r.MaxRetentionHours != 24 {
		return nil, newErrorResponse(http.StatusBadRequest, requestID(ctx))
	}
	consent, err := h.service.CreateConsent(ctx, input.Session, model.CreateConsentInput{Purpose: r.Purpose, Category: r.Category, PolicyVersion: r.PolicyVersion, ActivelyAgreed: r.ActivelyAgreed, TrainingAllowed: r.TrainingAllowed})
	if err != nil {
...
func (h *MediaHandler) createUpload(ctx context.Context, input *createMediaUploadInput) (*mediaUploadOutput, error) {
	...
	r := input.Body
	upload, err := h.service.CreateMediaUpload(ctx, input.Session, model.CreateMediaUploadInput{ConsentID: r.ConsentID, Purpose: r.Purpose, ContentType: r.ContentType, ByteSize: r.ByteSize, SHA256: r.SHA256})
	if err != nil {
		return nil, h.mediaError(ctx, err)
	}
...
func (h *MediaHandler) completeUpload(ctx context.Context, input *completeMediaUploadInput) (*mediaOutput, error) {
	...
	media, err := h.service.CompleteMediaUpload(ctx, input.Session, input.ID, model.CompleteMediaUploadInput{VersionID: input.Body.VersionID})
	if err != nil {
		return nil, h.mediaError(ctx, err)
	}
...
func (h *MediaHandler) mediaError(ctx context.Context, err error) error {
	...
	case errors.Is(err, model.ErrInvalidMediaInput):
		return newErrorResponse(http.StatusBadRequest, requestID(ctx))
	case errors.Is(err, model.ErrMediaTooLarge):
	...
	case errors.Is(err, model.ErrConsentRequired), errors.Is(err, model.ErrMediaConflict):
...
func consentResponse(c model.ConsentRecord) ConsentResponse {
...
func mediaResponse(m model.MediaAsset) MediaResponse {
...
func deletionResponse(d model.DeletionRequest) DeletionRequestResponse {
