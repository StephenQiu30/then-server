...
type WearEventHTTPService interface {
	CreateWearEvent(context.Context, string, string, model.WearEventInput) (model.WearEvent, error)
	...
	UpdateWearEvent(context.Context, string, string, int, model.WearEventInput) (model.WearEvent, error)
...
type WearEventFieldsRequest struct {
	...
	Completeness            model.WearEventCompleteness  `json:"completeness" enum:"partial,complete"`
	...
	SourceKind              model.WearEventSourceKind    `json:"source_kind" enum:"followed_plan,changed_plan,different_outfit,unplanned"`
...
func registerWearEventOperations(api huma.API, handler *WearEventHandler) {
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "createWearEvent", Method: http.MethodPost, Path: "/v1/wear-events", Tags: []string{"Wear Events"}, Summary: "记录本人实际穿着并生成服务端快照", DefaultStatus: http.StatusCreated, MaxBodyBytes: 20 * 1024, Errors: wearEventErrors(http.StatusBadRequest, http.StatusConflict)}), handler.create)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "listWearEvents", Method: http.MethodGet, Path: "/v1/wear-events", Tags: []string{"Wear Events"}, Summary: "按本地日期稳定分页列出本人实际穿着", Errors: wearEventErrors(http.StatusBadRequest, http.StatusNotFound)}), handler.list)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "getWearEvent", Method: http.MethodGet, Path: "/v1/wear-events/{wear_event_id}", Tags: []string{"Wear Events"}, Summary: "读取本人实际穿着与快照", Errors: wearEventErrors(http.StatusBadRequest, http.StatusNotFound)}), handler.get)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "updateWearEvent", Method: http.MethodPut, Path: "/v1/wear-events/{wear_event_id}", Tags: []string{"Wear Events"}, Summary: "按 revision 纠正实际穿着", MaxBodyBytes: 20 * 1024, Errors: wearEventErrors(http.StatusBadRequest, http.StatusNotFound, http.StatusConflict)}), handler.update)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "deleteWearEvent", Method: http.MethodDelete, Path: "/v1/wear-events/{wear_event_id}", Tags: []string{"Wear Events"}, Summary: "永久删除实际穿着并重算计划状态", DefaultStatus: http.StatusNoContent, Errors: wearEventErrors(http.StatusBadRequest, http.StatusNotFound, http.StatusConflict)}), handler.delete)
}
...
func (h *WearEventHandler) error(ctx context.Context, err error) error {
	switch {
	case errors.Is(err, model.ErrInvalidWearEventInput):
		return newErrorResponse(http.StatusBadRequest, requestID(ctx))
	case errors.Is(err, model.ErrWearEventNotFound):
		return newErrorResponse(http.StatusNotFound, requestID(ctx))
	case errors.Is(err, model.ErrWearEventDuplicate):
		...
		return response
	case errors.Is(err, model.ErrWearEventConflict):
		response := newErrorResponse(http.StatusConflict, requestID(ctx))
		response.Code, response.Message = "CONFLICT", "Wear event, outfit plan, or wardrobe facts changed."
		return response
	case errors.Is(err, model.ErrWearEventItemsUnavailable):
		response := newErrorResponse(http.StatusConflict, requestID(ctx))
...
func wearEventFields(request WearEventFieldsRequest) model.WearEventInput {
	items := make([]model.OutfitSelection, 0, len(request.Items))
	for _, item := range request.Items {
		items = append(items, model.OutfitSelection{ItemID: item.ItemID, Revision: item.Revision})
	}
	confirmations := make([]model.WearEventCandidate, 0, len(request.DuplicateConfirmations))
	for _, candidate := range request.DuplicateConfirmations {
		confirmations = append(confirmations, model.WearEventCandidate{ID: candidate.ID, Revision: candidate.Revision})
	}
	return model.WearEventInput{LocalDate: request.LocalDate, TimeZone: request.TimeZone, Completeness: request.Completeness, ContextSummary: request.ContextSummary, Items: items, LaundryItemIDs: request.LaundryItemIDs, ConfirmedUnavailableIDs: request.ConfirmedUnavailableIDs, SourcePlanID: request.SourcePlanID, SourcePlanRevision: request.SourcePlanRevision, SourceKind: request.SourceKind, DuplicateConfirmations: confirmations}
}

func wearEventResponse(event model.WearEvent) WearEventResponse {
