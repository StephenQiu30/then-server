...
func registerWardrobeOperations(api huma.API, handler *WardrobeHandler) {
	...
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "createWardrobeItem", Method: http.MethodPost, Path: "/v1/wardrobe/items", Tags: []string{"Wardrobe"}, Summary: "创建本人无图结构化衣物", DefaultStatus: http.StatusCreated, MaxBodyBytes: 8 * 1024, Errors: wardrobeErrors(http.StatusBadRequest, http.StatusConflict)}), handler.create)
...
func (h *WardrobeHandler) create(ctx context.Context, input *createWardrobeItemInput) (*wardrobeItemOutput, error) {
	...
	item, err := h.service.CreateWardrobeItem(ctx, input.Session, model.CreateWardrobeItemInput{ID: input.Body.ID, Name: input.Body.Name, Category: input.Body.Category, Availability: input.Body.Availability, Source: input.Body.Source})
	if err != nil {
...
func (h *WardrobeHandler) update(ctx context.Context, input *updateWardrobeItemInput) (*wardrobeItemOutput, error) {
	...
	item, err := h.service.UpdateWardrobeItem(ctx, input.Session, input.ID, input.Body.ExpectedRevision, model.UpdateWardrobeItemInput{Name: input.Body.Name, Category: input.Body.Category, Availability: input.Body.Availability})
	if err != nil {
		return nil, h.error(ctx, err)
	}
	return &wardrobeItemOutput{RequestID: requestID(ctx), Body: wardrobeResponse(item)}, nil
...
func wardrobeResponse(item model.WardrobeItem) WardrobeItemResponse {
	return WardrobeItemResponse{ID: item.ID, Name: item.Name, Category: item.Category, Availability: item.Availability, Source: item.Source, Revision: item.Revision, CreatedAt: item.CreatedAt, UpdatedAt: item.UpdatedAt}
}
