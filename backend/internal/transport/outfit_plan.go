...
func registerOutfitPlanOperations(api huma.API, handler *OutfitPlanHandler) {
	...
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "createOutfitPlan", Method: http.MethodPost, Path: "/v1/outfit-plans", Tags: []string{"Outfit Plans"}, Summary: "创建本人真实衣物穿搭计划", DefaultStatus: http.StatusCreated, MaxBodyBytes: 16 * 1024, Errors: outfitPlanErrors(http.StatusBadRequest, http.StatusConflict)}), handler.create)
	huma.Register(api, authenticatedOperation(huma.Operation{OperationID: "listOutfitPlans", Method: http.MethodGet, Path: "/v1/outfit-plans", Tags: []string{"Outfit Plans"}, Summary: "按本地日期稳定分页列出本人计划", Errors: errorsWithNotFound}), handler.list)
