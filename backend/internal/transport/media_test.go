...
func TestMediaStatusRequiresCookieAndHidesObjectReferences(t *testing.T) {
	...
	router, err := NewRouter(context.Background(), false, probeFunc(func(context.Context) error { return nil }), nil, nil, time.Second, slog.New(slog.NewJSONHandler(io.Discard, nil)), NewMediaHandler(service, true))
	if err != nil {
		t.Fatal(err)
	}
