...
func TestNormalizedJPEGChecksDigestAndPixelBudget(t *testing.T) {
	...
	media := model.MediaAsset{RawObjectKey: "source.jpg", ObjectVersionID: "fixed-version", ByteSize: int64(len(data)), SHA256: digest}
...
func TestNormalizedJPEGRejectsAppendedSecondFrame(t *testing.T) {
	...
	runner := &Runner{objects: &workerObjectStoreStub{data: data}}
	media := model.MediaAsset{RawObjectKey: "source.jpg", ObjectVersionID: "fixed-version", ByteSize: int64(len(data)), SHA256: fmt.Sprintf("%x", sha256.Sum256(data))}
