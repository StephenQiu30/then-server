...
func (b *Broker) Publish(ctx context.Context, event model.OutboxEvent) error {
...
func (b *Broker) Consume(ctx context.Context, queue string, handler func(context.Context, model.OutboxEvent) error) error {
	...
	for {
		...
		case delivery, open := <-deliveries:
			...
			event := model.OutboxEvent{ID: wire.ID, EventType: wire.EventType, AggregateID: wire.AggregateID, CreatedAt: delivery.Timestamp}
