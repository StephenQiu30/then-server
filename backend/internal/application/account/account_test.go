...
type accountRepositoryStub struct {
	...
	profileError    error
	profileInput    domain.PutProfileInput
	deletedSession  []byte
	deletedUser     string
}

func (r *accountRepositoryStub) CreateAccount(_ context.Context, user domain.User, hash string, session domain.Session) (domain.User, error) {
	r.createdUser, r.createdHash, r.createdSession = user, hash, session
	return user, nil
}

func (r *accountRepositoryStub) FindCredentialByEmail(context.Context, string) (domain.Credential, error) {
	return r.credential, r.credentialError
}

func (r *accountRepositoryStub) CreateSession(_ context.Context, session domain.Session) error {
	r.createdSession = session
	return nil
}

func (r *accountRepositoryStub) FindUserBySession(_ context.Context, _ []byte, _ time.Time) (domain.User, error) {
	return r.sessionUser, r.sessionError
}

func (r *accountRepositoryStub) UpdateUser(_ context.Context, _ string, expectedRevision int, email, displayName *string, now time.Time) (domain.User, error) {
...
func (r *accountRepositoryStub) FindProfileByUserID(context.Context, string) (domain.PublicProfile, error) {
	return r.profile, r.profileError
}

func (r *accountRepositoryStub) FindProfileByHandle(context.Context, string) (domain.PublicProfile, error) {
	return r.profile, r.profileError
}

func (r *accountRepositoryStub) PutProfile(_ context.Context, _ string, input domain.PutProfileInput, _ time.Time) (domain.PublicProfile, error) {
...
func TestRegisterNormalizesAndProtectsCredentials(t *testing.T) {
	...
	result, err := service.Register(context.Background(), domain.RegisterAccountInput{
	...
	if result.User.Status != domain.AccountActive || result.User.Role != domain.AccountUser || result.User.Revision != 1 {
...
func TestRegisterRejectsInvalidInputs(t *testing.T) {
	for _, input := range []domain.RegisterAccountInput{
		...
		if _, err := service.Register(context.Background(), input); !errors.Is(err, domain.ErrInvalidAccountInput) {
...
func TestLoginDoesNotRevealAccountExistence(t *testing.T) {
	...
	for _, repository := range []*accountRepositoryStub{
		{credentialError: domain.ErrAuthentication},
		{credential: domain.Credential{PasswordHash: string(wrongHash)}},
	} {
		service := newAccountServiceForTest(t, repository)
		if _, err := service.Login(context.Background(), domain.CreateSessionInput{Email: "person@example.test", Password: "correct-password"}); !errors.Is(err, domain.ErrAuthentication) {
...
func TestAuthenticatedUpdateLogoutAndDeleteUseSessionOwner(t *testing.T) {
	repository := &accountRepositoryStub{sessionUser: domain.User{ID: "user-id", Email: "old@example.test", DisplayName: "Old", Status: domain.AccountActive, Role: domain.AccountUser, Revision: 1}}
	service := newAccountServiceForTest(t, repository)
	registered, err := service.Register(context.Background(), domain.RegisterAccountInput{Email: "first@example.test", DisplayName: "First", Password: "correct-password"})
	if err != nil {
		t.Fatal(err)
	}
	email, name := " NEW@Example.Test ", " New Name "
	updated, err := service.UpdateCurrentUser(context.Background(), registered.Token, domain.UpdateCurrentUserInput{Email: &email, DisplayName: &name, ExpectedRevision: 1})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountUpdateRejectsStaleRevisionBeforeWriting(t *testing.T) {
	name := "New Name"
	repository := &accountRepositoryStub{sessionUser: domain.User{ID: "user-id", Revision: 3}}
	service := newAccountServiceForTest(t, repository)
	if _, err := service.UpdateCurrentUser(context.Background(), strings.Repeat("K", encodedTokenLength), domain.UpdateCurrentUserInput{
		DisplayName: &name, ExpectedRevision: 2,
	}); !errors.Is(err, domain.ErrAccountConflict) {
