...
func TestPutCurrentProfileNormalizesPublicFields(t *testing.T) {
	...
	repository := &accountRepositoryStub{
		sessionUser: domain.User{ID: "user-id", Revision: 1},
		profile:     domain.PublicProfile{Handle: "then_style", Revision: 1},
	}
	service := newAccountServiceForTest(t, repository)
	profile, err := service.PutCurrentProfile(context.Background(), strings.Repeat("K", encodedTokenLength), domain.PutProfileInput{
...
func TestPutCurrentProfileClearsBlankBio(t *testing.T) {
	...
	repository := &accountRepositoryStub{
		sessionUser: domain.User{ID: "user-id", Revision: 1},
		profile:     domain.PublicProfile{Handle: "then_style", Revision: 2},
	}
	service := newAccountServiceForTest(t, repository)
	if _, err := service.PutCurrentProfile(context.Background(), strings.Repeat("K", encodedTokenLength), domain.PutProfileInput{
...
func TestProfileValidationStopsBeforeRepositoryWrite(t *testing.T) {
	...
	for _, input := range []domain.PutProfileInput{
		{Handle: "a!", ExpectedRevision: 0},
		{Handle: "valid_handle", Bio: &tooLong, ExpectedRevision: 0},
		{Handle: "valid_handle", ExpectedRevision: -1},
	} {
		repository := &accountRepositoryStub{sessionUser: domain.User{ID: "user-id", Revision: 1}}
		service := newAccountServiceForTest(t, repository)
		if _, err := service.PutCurrentProfile(context.Background(), strings.Repeat("K", encodedTokenLength), input); !errors.Is(err, domain.ErrInvalidProfileInput) {
...
func TestPublicProfileInvalidHandleUsesNotFoundBoundary(t *testing.T) {
	...
	if _, err := service.PublicProfile(context.Background(), "not valid"); !errors.Is(err, domain.ErrProfileNotFound) {
