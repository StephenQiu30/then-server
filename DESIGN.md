...
版本：2026-09-24。此文件是 Then Web 与 iOS 共同的视觉、组件状态和交互标准；两个仓库根目录文件必须字节一致。产品业务、隐私、数据和发布事实仍以对应 PRD、Design、Plan、Acceptance 为准。

## 设计方向

于是采用克制的黑白界面：接近白色的页面、纯白内容表面、深墨色主要行动、细分隔线、清楚的文字层级与足够留白。设计参考用户提供的 `vercel-DESIGN.md` 中的视觉规律，将其转成穿搭决策产品的界面；不复制 Vercel 的品牌、营销导航、开发者术语、定价卡或产品插图。真实衣物、Look 与用户确认的记录是内容中心。
